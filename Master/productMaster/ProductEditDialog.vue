    ...
    if (isEdit.value && props.editData) {
      ...
      form.value = {
        ...
        start_use_date: props.editData.start_use_date
          ? new Date(props.editData.start_use_date)
          : undefined,
      }
    } else {
...
const handleSubmit = async () => {
  ...
  saving.value = true
  try {
    // 处理日期格式，转换为日本时区的YYYY-MM-DD格式
    const formData = { ...form.value }
    if (formData.start_use_date) {
      if (formData.start_use_date instanceof Date) {
        // 使用日本时区
        const japanDate = new Date(formData.start_use_date.getTime() + 9 * 60 * 60 * 1000)
        formData.start_use_date = japanDate.toISOString().split('T')[0]
      } else if (typeof formData.start_use_date === 'string' && formData.start_use_date) {
        // 如果是ISO格式的字符串，先转换为Date对象再处理
        const date = new Date(formData.start_use_date)
        const japanDate = new Date(date.getTime() + 9 * 60 * 60 * 1000)
        formData.start_use_date = japanDate.toISOString().split('T')[0]
      }
    } else {
      // 如果没有选择日期，设置为null而不是空字符串
      formData.start_use_date = null
    }
