...
def is_welding_excel_watcher_enabled() -> bool:
    """溶接管理指標 Excel 監視が有効か（未設定は True）。Excel 計画監視とは独立。"""
    raw = _read_raw()
    return raw.get("welding_excel_watcher_enabled", True)


def is_welding_management_sync_enabled() -> bool:
