<template>
  <div class="help-page" :class="{ 'is-capturing': isCapturingPdf }">
    ...
    <el-card class="help-card" shadow="never">
      ...
      <div v-else ref="helpContentEl" class="help-content" v-html="renderedHtml"></div>
    </el-card>
    </div>
  </div>
</template>
...
async function handlePdfSave() {
  ...
  try {
    ...
    const fileName = '検査実績収集_操作説明.pdf'
...
/* 不良（項目別）デモ — 帰属工程ごとに1行・狭画面は横スクロール */
:deep(.help-content .insp-help-defect) {
  margin: 10px 0 14px;
  --insp-help-defect-col-w: 104px;
}

:deep(.help-content .insp-help-defect__panel) {
...
:deep(.help-content .insp-help-defect__head) {
  display: flex;
  flex-wrap: wrap;
...
:deep(.help-content .insp-help-defect__head strong) {
  font-size: 13px;
  font-weight: 800;
  color: #0f172a;
}

:deep(.help-content .insp-help-defect__head span) {
  font-size: 11px;
  color: #64748b;
}

:deep(.help-content .insp-help-defect__group) {
  overflow-x: auto;
  -webkit-overflow-scrolling: touch;
}

:deep(.help-content .insp-help-defect__group-head) {
  display: flex;
  align-items: baseline;
  gap: 6px;
  flex-wrap: wrap;
...
:deep(.help-content .insp-help-defect__group-label) {
  font-size: 10px;
  font-weight: 600;
  color: #64748b;
}

:deep(.help-content .insp-help-defect__group-name) {
  font-size: 12px;
  font-weight: 800;
  color: #0f766e;
}

:deep(.help-content .insp-help-defect__grid) {
  display: grid;
  grid-template-columns: repeat(var(--defect-cols, 7), var(--insp-help-defect-col-w));
  gap: 4px;
  width: max-content;
  min-width: min(100%, calc(var(--defect-cols, 7) * var(--insp-help-defect-col-w) + (var(--defect-cols, 7) - 1) * 4px));
  max-width: 100%;
}

:deep(.help-content .insp-help-defect__cell) {
  display: flex;
...
:deep(.help-content .insp-help-defect__cell span) {
  font-size: 10px;
  font-weight: 700;
  line-height: 1.25;
  color: #334155;
...
:deep(.help-content .insp-help-defect__cell em) {
  font-style: normal;
  font-size: 14px;
  font-weight: 800;
...
:deep(.help-content .insp-help-defect__cell--on) {
  border-color: #fbbf24;
  background: linear-gradient(180deg, #fffbeb 0%, #fef3c7 100%);
}

:deep(.help-content .insp-help-defect__cell--on em) {
  color: #b45309;
}

@media (max-width: 640px) {
  :deep(.help-content .insp-help-defect) {
    --insp-help-defect-col-w: 92px;
  }
}
