<template>
  <div class="right-gen-pane">
    <div class="pane-head pane-head--right">
      <div class="pane-toolbar">
        <span v-if="!hideToolbarTitle" class="pane-title">翌日の見込数量</span>
        <div class="pane-control-group pane-control-group--date">
          <span class="pane-inline-label">参照日</span>
          <el-date-picker
            :model-value="genDate"
            type="date"
            ...
            @update:model-value="emit('update:genDate', $event)"
          />
          <el-button
            class="pane-nav-btn"
            size="small"
    ...
    >
      ...
      <el-table-column label="必要治具本数" width="100" align="right">
        <template #default="{ row }">
          {{ calcRequiredJigCount(row.gen_qty, row.plating_efficiency) }}
        </template>
      </el-table-column>
    </el-table>
  </div>
...
defineProps<{
  ...
  calcRequiredJigCount: (qty: number | string, efficiency: string) => string | number
}>()

const emit = defineEmits<{
  'update:genDate': [value: string]
...
.pane-head--right {
  padding: 6px 8px;
  border-radius: 10px;
  border: 1px solid color-mix(in oklab, var(--el-color-success-light-5) 55%, var(--el-border-color-lighter));
  background: linear-gradient(
    165deg,
    color-mix(in oklab, var(--el-color-success-light-9) 92%, #fff) 0%,
    color-mix(in oklab, var(--el-color-success-light-9) 42%, var(--el-fill-color-blank)) 100%
  );
  box-shadow:
...
.pane-title {
  ...
  line-height: 1.2;
  white-space: nowrap;
  color: color-mix(in oklab, var(--el-color-success) 78%, var(--el-text-color-primary));
...
.pane-control-group {
  ...
  border-radius: 8px;
  border: 1px solid color-mix(in oklab, var(--el-border-color) 70%, var(--el-color-success-light-7));
...
.pane-nav-btn:hover {
  ...
  background: linear-gradient(180deg, #fff 0%, var(--el-color-success-light-9) 100%);
}
...
.pane-head-meta {
  ...
  padding: 3px 10px;
  border-radius: 999px;
  border: 1px solid color-mix(in oklab, var(--el-color-success-light-5) 50%, var(--el-border-color-lighter));
  background: linear-gradient(180deg, #fff 0%, var(--el-color-success-light-9) 120%);
  box-shadow:
