...
const handleCreateSchedule = () => {
  ElMessage.info('支払予定表作成画面を開きます')
}
const handleExportFb = () => {
  ElMessage.success('FBデータ（全銀協フォーマット）を出力しました')
}
const handleApprove = () => {
  ElMessage.success('支払を承認しました')
}
const handlePrint = () => {
  ElMessage.info('支払一覧を印刷します')
}
const handleView = (row: any) => {
  ElMessage.info(`支払 ${row.payment_no} の詳細`)
}

const isOverdue = (row: any) => row.status === 'pending' && new Date(row.due_date) < new Date()
const getStatusType = (s: string): ElTagType =>
  (({ pending: 'warning', approval: 'info', paid: 'success' }) as Record<string, ElTagType>)[s] ||
  'info'
const getStatusLabel = (s: string) =>
  ({ pending: '支払待ち', approval: '承認待ち', paid: '支払済' })[s] || s
</script>
