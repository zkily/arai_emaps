# Smart-EMAPs (ERP+APS+MES+FIN) 統合管理システム

製造業のデジタルトランスフォーメーションを実現する次世代統合管理システムです。ERP（企業資源計画）、APS（先進的計画・スケジューリング）、MES（製造実行システム）に加え、**FIN（経理・原価・人事）** を統合し、設計から製造、出荷、原価・会計・人事まで一気通貫のデータフローを構築します。

## 🎯 システム概要

Smart-EMAPsは、製造業における経営資源の最適化、高精度な生産計画、現場の実行と可視化、**製造原価・法人会計・債権債務・勤怠・給与** を統合的に実現します。各モジュールが緊密に連携し、リアルタイムなデータ共有により、迅速な意思決定と業務効率の最大化を支援します。
...
│   │   ├── modules/           # 機能モジュール（auth/erp/aps/mes/fin/master/system/…）
...
│       │   ├── erp/           # ERP画面（受注・販売・購買・在庫・出荷 等）
│       │   ├── fin/           # FIN画面（原価・会計・債権債務・経費・勤怠・給与・人事）
...
│   ├── FIN_MODULE_GUIDE.md    # FIN（経理・原価・人事）モジュール
│   ├── fin/                   # FIN ドメイン定義（fin-domain-manifest.yaml）
│   ├── WEBSOCKET_IMPLEMENTATION.md
│   └── （その他）操作説明・設計メモ
│
├── scripts/                    # 運用・生成スクリプト
│   ├── bootstrap_full_database.py
│   └── fin/                   # FIN コードジェネレータ（fin_codegen.py）
│
├── Master/                     # マスタデータファイル
├── logs/                       # ログファイル
├── start.py                    # 統合起動スクリプト
...
- **ルーティング**: `frontend/src/router` で画面ごとにルートを定義し、ERP / APS / MES / **FIN** / System などドメイン単位でモジュール分割
...
- **UI コンポーネント**:
  ...
  - `views/fin` 以下は原価管理・会計・売掛/買掛・経費・勤怠・給与・人事など FIN 領域（`finMenuConfig` + `MenuTreeItem` でサイドバー描画）
...
- **ドメインモジュール** (`backend/app/modules`):
  ...
  - `mes`: MES
  - `fin`: 経理・原価・人事（会計・債権債務・経費・勤怠・給与・人事・ERP 連携仕訳）
...
### 💰 5. FIN：経理・原価・人事 (Finance, Costing & HR)

ERP の製造原価と法人会計・人事を **FIN 顶级モジュール** に集約。サイドバーでは **MES の下・マスタの上**（`sortOrder=3.5`）に配置。詳細は [docs/FIN_MODULE_GUIDE.md](docs/FIN_MODULE_GUIDE.md) を参照。

#### 原価管理（`views/fin/costing`）

旧 `views/erp/costing` から移管。URL は `/fin/costing/*`（旧 `/erp/costing/*` は自動リダイレクト）。

- **標準原価計算** / **実際原価計算** / **原価差異分析**
- **配賦計算**（労務費・製造経費の製品別配賦）
- **仕掛品(WIP)評価**（月末工程内在庫）
- **設備台帳** / **減価償却計算**（製造原価連携）
...
- **仕訳帳**・**総勘定元帳**・**試算表**・**貸借対照表/PL**・**月次・年次決算**
- **仕訳取込・連携**（ERP イベントからの自動仕訳ソース）
- **売上・請求（AR）**：請求書・入金消込・売掛年齢表
- **仕入・支払（AP）**：仕入請求・支払・買掛年齢表

#### 経費・勤怠・給与・人事

- **経費精算**：申請・承認・経費区分・規程
- **勤怠**：勤怠記録・休暇申請・シフト・勤務カレンダー
- **給与**：給与計算・明細・振込データ・支給控除項目
- **人事**：社員・雇用契約・年末調整
- **経理マスタ**：勘定科目・税区分・取引先・銀行口座
- **従業員ポータル** / **設定**（会計設定・承認フロー）

#### ERP → FIN 連携

製造・販売・仕入などの業務イベントを `fin_journal_source` に記録し、月次で仕訳（draft）へ一括生成。例：売上計上 → 借方売掛金 / 貸方売上高。API プレフィックスは **`/api/fin`**。

#### 技術構成（コードジェネレータ）

| 成果物 | パス |
|--------|------|
| ドメイン定義（単一ソース） | `docs/fin/fin-domain-manifest.yaml` |
| ジェネレータ | `scripts/fin/fin_codegen.py`（`py -3 scripts/fin/fin_codegen.py --target all`） |
| DB スキーマ（41 表 + 種子） | `backend/database/migrations/36_fin_full_schema.sql` |
| バックエンド | `backend/app/modules/fin/` |
| フロント画面 | `frontend/src/views/fin/` |
| メニュー / ルート | `frontend/src/router/finMenuConfig.ts`、`frontend/src/shared/router/finRoutes.ts`、`finCostingRoutes.ts` |

> **メニュー同期**: 新規ルート追加後は **システム管理 → メニュー管理 → ルート定義から取り込み** を実行してください。権限モジュール名は **`経理・原価・人事`** です。
...
- **システム管理**
  ...
  - API連携監視（ERP⇔APS⇔MES⇔FIN 間のデータ転送ログ・エラー通知）
  - 多言語・多通貨対応（海外拠点展開用）
...
4. **原価計算** (FIN) → 標準/実際原価・差異分析（`/fin/costing`）
5. **会計・債権債務** (FIN) → 仕訳・試算表・請求/支払（`/fin/*`）
...
上述脚本会：`CREATE DATABASE IF NOT EXISTS`（与 `DB_NAME` 一致）→ 执行 `backend/database/init/01_init.sql` → 按编号顺序执行 `backend/database/migrations/*.sql`（**FIN 模块含 `36_fin_full_schema.sql`**）。仅预览不执行可加 `--dry-run`；**开发空库**需要整库重建时可加 `--drop-database`（会 `DROP DATABASE`，危险）。
...
- **言語**: 既定は日本語。`frontend/src/locales` に **日本語（ja）・中国語（zh）・ベトナム語（vi）** 等のリソースがあり、Vue I18n で切替可能。FIN メニューは `frontend/src/locales/fin/` のフラグメントをマージ（`menu.FIN_*` キー）。
