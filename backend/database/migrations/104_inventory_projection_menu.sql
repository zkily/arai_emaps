-- 生産計画 > 月末在庫予測（menuConfig / SidebarMenu と整合）

INSERT IGNORE INTO menus (code, name, parent_id, path, icon, sort_order)
SELECT 'ERP_PRODUCTION_INVENTORY_PROJECTION', '月末在庫予測', m.id, '/erp/production/inventory-projection', 'DataLine', 5
FROM menus m
...
SET child.name = '月末在庫予測',
    child.path = '/erp/production/inventory-projection',
    child.icon = 'DataLine',
    child.sort_order = 5,
    child.parent_id = parent.id,
    child.is_active = 1
WHERE child.code = 'ERP_PRODUCTION_INVENTORY_PROJECTION';
...
INNER JOIN menus new_menu ON new_menu.code = 'ERP_PRODUCTION_INVENTORY_PROJECTION';
...
WHERE NOT EXISTS (
  ...
  WHERE m.code = 'ERP_PRODUCTION_INVENTORY_PROJECTION'
);

INSERT IGNORE INTO role_menu_permissions (role_id, menu_id)
SELECT (SELECT id FROM roles WHERE name = '管理者' LIMIT 1), id
FROM menus
WHERE code = 'ERP_PRODUCTION_INVENTORY_PROJECTION';
