...
CREATE TABLE IF NOT EXISTS `welding_management` (
  ...
  `mes_operator_user_id` int NULL DEFAULT NULL COMMENT 'MES溶接作業者(users.id)',
  `mes_client_instance_id` varchar(64) NULL DEFAULT NULL COMMENT 'MES操作端末ID（localStorage UUID）',
  `remarks` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL COMMENT '備考',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '作成日時',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新日時',
  PRIMARY KEY (`id`),
  INDEX `idx_welding_production_day` (`production_day`),
  INDEX `idx_welding_product_cd` (`product_cd`),
  INDEX `idx_welding_production_month` (`production_month`),
  INDEX `idx_welding_completed` (`production_completed_check`)
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '溶接指示・MES実績' ROW_FORMAT = Dynamic;
