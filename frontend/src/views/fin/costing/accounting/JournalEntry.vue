      ...
      >
        ...
        <el-table-column prop="type" label="種別" width="100" align="center">
          <template #default="{ row }">
            <el-tag size="small">
              {{ journalTypeLabel(String(row.type)) }}
            </el-tag>
          </template>
        </el-table-column>
...
const journalTypeLabel = (t: string) => JOURNAL_TYPE_LABELS[t] ?? t
...
const handleGenerate = () => {
  ElMessage.info('自動仕訳生成を実行します')
}
const handleSelectionChange = (rows: any[]) => {
  selectedRows.value = rows
}
const handlePost = () => {
  ElMessage.info(`${selectedRows.value.length}件の仕訳を転記します`)
}
const handleExport = () => {
  ElMessage.info('仕訳データをエクスポートします')
}
const handleView = (row: any) => {
  ElMessage.info(`仕訳 ${row.journal_no} の詳細`)
}
const getStatusType = (s: string): ElTagType =>
  (({ pending: 'info', generated: 'primary', posted: 'success' }) as Record<string, ElTagType>)[s] ||
  'info'
const getStatusLabel = (s: string) =>
  ({ pending: '未生成', generated: '生成済', posted: '転記済' })[s] || s
</script>
