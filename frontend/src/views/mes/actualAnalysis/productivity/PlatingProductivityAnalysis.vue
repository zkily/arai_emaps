    ...
    <header class="ipa-hero ipa-fade-in">
      <div class="ipa-hero__main">
        ...
        <div class="ipa-hero__text">
          ...
          <h1 class="ipa-hero__title">メッキ工程 — 生産性分析</h1>
...
import {
  fetchPlatingProductivityAnalysis,
  type PlatingProductivityAnalysisData,
  type PlatingProductivityBucket,
  type PlatingProductivityDailyRow,
  type PlatingProductivityOperatorRow,
  type PlatingProductivityProductOperatorRanking,
  type PlatingProductivityProductRow,
  type PlatingProductivitySessionRow,
} from '@/api/platingProductionIndicator'
import type { PlatingProductionIndicatorLineOption as PlatingLineOption } from '@/api/platingProductionIndicator'
import { getProductList } from '@/api/master/productMaster'
import type { Product } from '@/types/master'
import { getJSTToday, parseDateAsJST } from '@/utils/dateFormat'
import { lineLabel, loadPlatingLineOptions, resolvePlatingMetricsLabel } from '@/views/mes/shared/platingLineFilter'
import { filterPlatingSelectableProducts } from '@/views/mes/shared/platingProductFilter'
import {
  CHART_INIT_OPTS,
  chartTheme,
...
import {
  printPlatingProductivityDailyBatch,
  printPlatingProductivityOperatorProductBatch,
  printPlatingProductivityReport,
  printPlatingProductivitySection,
  type PlatingProductivityDailyBatchItem,
  type PlatingProductivityOperatorProductBatchItem,
  type PlatingProductivityPrintSection,
  type PlatingProductivityReportContext,
  type PlatingProductivityReportFilters,
} from './platingProductivityReport'

defineOptions({ name: 'MesPlatingProductivityAnalysis' })
...
const analysisData = ref<PlatingProductivityAnalysisData | null>(null)
const lineOptions = ref<PlatingLineOption[]>([])
...
const emptySummary = (): PlatingProductivityBucket => ({
  session_count: 0,
...
const productRankList = computed((): PlatingProductivityProductOperatorRanking[] => {
...
function buildProductOperatorRankingFromSessions(
  sessions: PlatingProductivityAnalysisData['sessions'],
  ...
  const productMap = new Map<
    string,
    {
      ...
      operators: Map<string, PlatingProductivityOperatorRow>
    }
  >()
  ...
  const result: PlatingProductivityProductOperatorRanking[] = []
  for (const prod of productMap.values()) {
    const operators: PlatingProductivityOperatorRow[] = []
...
const podiumOperators = computed(() => {
  ...
  return order
    ...
    .filter((r): r is PlatingProductivityOperatorRow => r != null)
})
...
function productRankOptionLabel(p: PlatingProductivityProductOperatorRanking): string {
...
function sortByEfficiency(a: PlatingProductivityOperatorRow, b: PlatingProductivityOperatorRow): number {
...
type OperatorDisplayRow = PlatingProductivityOperatorRow & {
  avg_efficiency_per_hour: number | null
}

type ProductDisplayRow = PlatingProductivityProductRow & {
  avg_efficiency_per_hour: number | null
}

/** 期間内：生産合計 ÷ 正味稼働時間（個/時） */
function periodAvgEfficiencyFromBucket(row: PlatingProductivityBucket): number | null {
...
function sortByOperatorDefectRate(a: PlatingProductivityOperatorRow, b: PlatingProductivityOperatorRow): number {
...
function rankOverviewRowClass({ row }: { row: PlatingProductivityProductOperatorRanking }): string {
...
function buildOperatorProductRows(
  ...
  const map = new Map<string, PlatingProductivityProductRow & { sum_net_production_sec: number }>()
  for (const s of sessions) {
...
function buildReportFilters(): PlatingProductivityReportFilters | null {
...
function buildReportContext(): PlatingProductivityReportContext | null {
...
async function handleReportCommand(command: string | number | object) {
  ...
  try {
    ...
    if (cmd === 'print-full') {
      printPlatingProductivityReport(ctx)
      return
    }

    const sectionMap: Record<string, PlatingProductivityPrintSection> = {
      'print-daily': 'daily',
    ...
    if (section) {
      printPlatingProductivitySection(section, ctx)
      return
    }
  } catch (err) {
...
async function captureDailyChartDataUrl(daily: PlatingProductivityDailyRow[]): Promise<string | null> {
...
async function handleBatchDailyByOperatorPrint() {
  ...
  const items: PlatingProductivityDailyBatchItem[] = []
  for (const [idx, op] of operators.entries()) {
    const res = await fetchPlatingProductivityAnalysis({
      start_date: start,
      end_date: end,
  ...
  printPlatingProductivityDailyBatch(filters, items)
}
...
async function handleBatchOperatorProductPrint() {
  ...
  const items: PlatingProductivityOperatorProductBatchItem[] = []
  ...
  printPlatingProductivityOperatorProductBatch(filters, items)
}
...
async function loadProductOptions() {
  ...
  try {
    ...
    productOptions.value = filterPlatingSelectableProducts(list)
  } catch {
...
function defectLabel(defectCd: string): string {
  ...
  return defectLabelMap.value.get(cd) ?? resolvePlatingMetricsLabel(cd, defectLabelMap.value)
}
...
async function loadLines() {
  ...
  try {
    lineOptions.value = await loadPlatingLineOptions({ start_date: start, end_date: end })
