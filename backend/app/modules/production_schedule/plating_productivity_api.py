# coding: utf-8
"""メッキ工程 生産性分析 API（plating_production_indicator 集計）"""
...
def _plating_row_work_sec(item: dict[str, Any]) -> int:
...
def _plating_row_paused_sec(item: dict[str, Any]) -> int:
    shift_h = _to_float(item.get("shift_hours"))
    work_h = _to_float(item.get("work_hours"))
    if shift_h > work_h > 0:
        return max(0, int(round((shift_h - work_h) * 3600)))
    pause_h = sum(_to_float(item.get(field)) for _, field in PLATING_LINE_METRICS_LOSS_FIELDS)
    if pause_h > 0:
        return max(0, int(round(pause_h * 3600)))
    return 0


def _normalize_plating_row(row: dict[str, Any]) -> dict[str, Any]:
    item = dict(row)
...
def _new_line_metrics_bucket(*, line_key: str, line_name: str) -> dict[str, Any]:
    ...
    for header, _field in PLATING_LINE_METRICS_LOSS_FIELDS:
...
def register_routes(router: APIRouter) -> None:
    ...
    @router.get("/plan/plating-production-indicator/productivity-analysis")
    async def get_plating_productivity_analysis(
        start_date: str = Query(..., description="開始日 YYYY-MM-DD"),
        end_date: str = Query(..., description="終了日 YYYY-MM-DD"),
        ...
        """メッキ能率分析（plating_production_indicator 集計）"""
        start_d = _parse_date_ymd(start_date)
        end_d = _parse_date_ymd(end_date)
        ...
        try:
            ...
            if "plating_production_indicator" in msg and (
                ...
                raise HTTPException(
                    status_code=503,
                    detail="plating_production_indicator テーブルが存在しません。backend/database/migrations/73_plating_production_indicator.sql を実行してください",
                ) from e
        ...
        for row in rows:
            item = _normalize_plating_row(dict(row))
            ...
            net_sec = _plating_row_work_sec(item)
            paused_sec = _plating_row_paused_sec(item)
            ...
            stop_h = sum(_to_float(item.get(field)) for _, field in PLATING_LINE_METRICS_LOSS_FIELDS)
