  ...
  <div class="outsourcing-order-page">
          ...
          <el-form :model="batchFormData" label-width="70px" class="batch-dialog__form" label-position="left">
            <div class="batch-dialog__form-row">
              ...
              <el-form-item label=" " class="batch-dialog__form-item batch-dialog__form-item--action">
                ...
                >
                  <el-icon v-if="!batchProductLoading"><Download /></el-icon>
                  {{ batchProductLoading ? '読込中...' : '読込' }}
                </el-button>
              </el-form-item>
            </div>
          </el-form>
          ...
          <div class="batch-dialog__table-wrap">
            ...
            >
              <el-table-column
                ...
                fixed="left"
              />
              <el-table-column prop="productCd" label="製品CD" width="92" align="center" />
              <el-table-column
                prop="productName"
                label="製品名"
                min-width="120"
    ...
    <el-dialog v-model="detailVisible" title="注文詳細" width="700px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="溶接点数">{{ detailData.weldingPoints }}</el-descriptions-item>
            ...
            <div class="form-fields-compact">
              ...
              <div class="form-field-row">
                ...
                <el-input
                  ...
                  type="textarea"
                  :rows="2"
                  placeholder="3.支払期日・支払方法・検査完了期日・有償支給原材料代金の決済期日及び方法については、令和8年7月1日の「支払方法等について」によります。"
                  class="form-textarea-compact"
                  size="small"
                />
              </div>
...
const convertOrderFromBackend = (item: any): OrderItem => {
  ...
  return {
    ...
    weldingPoints: item.welding_points || item.weldingPoints || 0,
    quantity,
    unitPrice,
    amount,
...
// 数据转换：前端camelCase -> 后端snake_case（编辑/提交时使用）
const _convertOrderToBackend = (item: Partial<OrderItem>, includeStatus = false): any => {
  const data: any = {
    supplier_cd: item.supplierCd ?? undefined,
    order_date: item.orderDate,
    product_cd: item.productCode,
    product_name: item.productName,
    welding_type: item.weldingType,
    welding_points: item.weldingPoints || 0,
...
// 日本货币格式化（带小数）（打印等场景可用）
const _formatCurrencyDecimal = (value: number | null | undefined): string => {
...
const handleSearch = async () => {
  ...
  try {
    ...
    const res = await getWeldingOrders(params)
    // request 拦截器已返回 response.data，故 res 即为 { success, data, total }
    const responseData =
      res && typeof res === 'object' && ('success' in res || 'data' in res)
        ? (res as unknown as { success?: boolean; data?: unknown[]; total?: number })
        : Array.isArray(res)
          ? { success: true, data: res, total: (res as unknown[]).length }
          : null

    if (responseData?.success) {
      const data = responseData.data
      const list = Array.isArray(data) ? data : []
      orderList.value = list.map(convertOrderFromBackend)
...
const loadBatchProductOptions = async () => {
  ...
  try {
    ...
    if (Array.isArray(body)) {
      ...
      products = (body as { data: unknown[] }).data
    }
...
const fetchBatchProducts = async () => {
  ...
  try {
    ...
    const listRes = await getWeldingOrders({
      page: 1,
      pageSize: 500,
    ...
    for (const o of existingOrders) {
      ...
      if (existingByDate[orderDateStr]) {
        ...
        existingByDate[orderDateStr] = {
          quantity: qty,
          deliveryDate: deliveryStr || addBusinessDays(orderDateStr, Number(selectedProduct.delivery_lead_time || selectedProduct.deliveryLeadTime || 7)),
        }
      }
    }
    ...
    batchOrderList.value = dateList.map((orderDate) => {
      ...
      return {
        ...
        productName: selectedProduct.product_name || selectedProduct.productName || '',
        weldingType: selectedProduct.welding_type || selectedProduct.weldingType || '溶接',
        weldingPoints: Number(selectedProduct.welding_points || selectedProduct.weldingPoints || 0),
...
const submitBatchForm = async () => {
  ...
  try {
    ...
    // 批量创建订单（order_date/delivery_date 保持字符串 YYYY-MM-DD，quantity 为整数）
    ...
    const orderDataList = validOrders.map((order) => {
      const quantity =
        typeof order.quantity === 'string' ? parseFloat(order.quantity) : Number(order.quantity)
      const qty = isNaN(quantity) || quantity < 1 ? 1 : Math.floor(quantity)
      const orderDate = toDateStr(order.orderDate)
      const deliveryDate = order.deliveryDate != null ? toDateStr(order.deliveryDate) : undefined
      return {
        supplier_cd: batchFormData.supplierCd,
        order_date: orderDate,
        product_cd: order.productCd,
        product_name: order.productName,
        welding_type: order.weldingType || '溶接',
        welding_points: Number(order.weldingPoints) || 0,
        quantity: qty,
        unit: '個',
    ...
    // 一括登録（1リクエストで連番採番し重複を防ぐ）
    await createWeldingOrdersBatch(orderDataList)
...
const editOrder = async (row: OrderItem) => {
  ...
  try {
    ...
    Object.assign(editFormData, {
      ...
      weldingPoints: order.weldingPoints || 0,
...
const loadHolidays = async (destinationCd?: string) => {
  try {
    ...
    // 兼容: 空时返回 { data: [] }; axios 时 res.data 为响应体(数组或 { data: [] })
    const raw = (res as { data?: unknown } | undefined)?.data
    let holidays: { holiday_date?: string; holidayDate?: string }[] = []
    if (Array.isArray(raw)) {
      holidays = raw
    } else if (raw && typeof raw === 'object' && Array.isArray((raw as { data?: unknown[] }).data)) {
      holidays = (raw as { data: { holiday_date?: string; holidayDate?: string }[] }).data
    }
  ...
  try {
    ...
    productList.value = products
      ...
      .map((p) => ({
        productCd: p.product_cd || p.productCode || '',
        productName: p.product_name || p.productName || '',
        weldingType: p.welding_type || p.weldingType || '溶接',
        weldingPoints: Number(p.welding_points || p.weldingPoints || 0),
  ...
  try {
    ...
    const orderDataList = validProducts.map((product) => {
      ...
      return {
        ...
        welding_type: product.weldingType || '溶接',
        welding_points: product.weldingPoints || 0,
    ...
    for (const orderData of orderDataList) {
      ...
      await createWeldingOrder(orderData as any)
    }
...
const deleteOrder = async (row: OrderItem) => {
  ...
  try {
    ...
    await deleteWeldingOrder(row.id)
...
const submitEditForm = async () => {
  ...
  submitLoading.value = true
  try {
    // 将 undefined 转换为 null，因为 MySQL 不接受 undefined
    ...
    const updateData = {
      ...
      welding_type: editFormData.weldingType || null,
      welding_points: editFormData.weldingPoints || 0,
      quantity: editFormData.quantity,
      unit: '個', // 添加单位字段
      unit_price: editFormData.unitPrice,
      delivery_date: editFormData.deliveryDate || null,
      delivery_location: safeValue(editFormData.deliveryLocation),
      category: safeValue(editFormData.category),
      content: safeValue(editFormData.content),
      specification: safeValue(editFormData.specification),
      status: editFormData.status || null,
      remarks: safeValue(editFormData.remarks),
    }
        ...
        <head>
          ...
          .title {
            ...
            th, td {
              ...
              padding: 3.6mm 3.6mm; /* 行高增加50% (原2.4mm * 1.5 = 3.6mm) */
              text-align: left;
              font-size: 9.5pt;
            }
            th {
...
const generatePrintHtml = (orderItems: OrderItem[]) => {
  ...
  groupKeys.forEach((groupKey, _index) => {
...
.batch-dialog__select :deep(.el-input__wrapper:hover),
.batch-dialog__date-picker :deep(.el-input__wrapper:hover) {
...
.batch-dialog__qty-input--filled :deep(.el-input__wrapper) {
...
.batch-dialog__btn--cancel {
  border: 1px solid #dcdfe6;
  background: #fff;
  color: #606266;
}

.batch-dialog__btn--cancel:hover {
  border-color: #c0c4cc;
  background: #f5f7fa;
  color: #303133;
}
