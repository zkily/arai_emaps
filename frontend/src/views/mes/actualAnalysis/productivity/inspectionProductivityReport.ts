import type {
  ...
  InspectionProductivitySessionRow,
} from '@/api/inspectionManagement'

export interface InspectionProductivityReportFilters {
  startDate: string
  endDate: string
...
export interface InspectionProductivityWeldRankRow {
  rank: number
...
export type InspectionProductivityKpiTone = 'indigo' | 'sky' | 'amber' | 'emerald' | 'violet'

export interface InspectionProductivityKpiCardPrint {
  label: string
  value: string
  hint: string
  tone: InspectionProductivityKpiTone
}

export interface InspectionProductivityPrintCharts {
  daily: string | null
  inspector: string | null
  product: string | null
}

export interface InspectionProductivityPrintChartsExtended extends InspectionProductivityPrintCharts {
  productRank?: string | null
}

export interface InspectionProductivityProductRankPrintContext {
  selected: InspectionProductivityProductInspectorRanking | null
  topOverview: InspectionProductivityProductInspectorRanking[]
  stats: {
...
export interface InspectionProductivityReportContext {
  filters: InspectionProductivityReportFilters
  kpiCards: InspectionProductivityKpiCardPrint[]
  charts: InspectionProductivityPrintChartsExtended
  inspectorRows: Array<InspectionProductivityInspectorRow & { avg_efficiency_per_hour?: number | null }>
  productRows: Array<InspectionProductivityProductRow & { avg_efficiency_per_hour?: number | null }>
  inspectorSectionAvgEfficiency: number | null
  productSectionTotalQty: number
  weldRankOff: InspectionProductivityWeldRankRow[]
  weldRankOn: InspectionProductivityWeldRankRow[]
  productRank: InspectionProductivityProductRankPrintContext
}

export type InspectionProductivityPrintSection =
  | 'full'
  | 'daily'
  | 'inspector'
  | 'product'
...
/** @deprecated use InspectionProductivityReportContext */
export type InspectionProductivityPrintPayload = InspectionProductivityReportContext
...
function reportFileBase(filters: InspectionProductivityReportFilters): string {
  return `検査生産性分析_${filters.startDate}_${filters.endDate}`
}

function metaCsvLines(filters: InspectionProductivityReportFilters): string[] {
  return [
    csvLine(['# 検査工程 — 生産性分析']),
    csvLine(['# 集計期間', `${filters.startDate} ～ ${filters.endDate}`]),
    csvLine(['# 検査員', filters.inspectorLabel]),
...
export function buildSessionsCsv(
  ...
  for (const row of data.sessions) {
    lines.push(
      csvLine([
        row.production_day ?? '',
        row.inspector_display_name ?? row.mes_inspector_name ?? '',
        row.product_cd ?? '',
        row.product_name ?? '',
...
export function buildSummaryCsv(
  ...
  extras: {
    ...
    weldRankOff: InspectionProductivityWeldRankRow[]
    weldRankOn: InspectionProductivityWeldRankRow[]
    productRankTop: InspectionProductivityProductInspectorRanking[]
  ...
  lines.push(csvLine(['日付', '件数', '確定件数', '生産数', '不良数', '不良率', '能率(本/時)', '正味稼働(分)']))
  ...
  lines.push(csvLine(['【検査員別】']))
  lines.push(csvLine(['検査員', '件数', '生産数', '不良率', '平均能率(本/時)', '正味稼働(分)']))
  for (const row of extras.inspectorRows) {
    lines.push(
      csvLine([
        row.inspector_name ?? '',
        row.session_count ?? 0,
  ...
  lines.push(csvLine(['CD', '製品名', '件数', '生産数', '不良率', '平均能率(本/時)']))
  ...
  lines.push(csvLine(['【溶接工程なし — 検査員平均能率ランキング】']))
  lines.push(csvLine(['順位', '検査員', '件数', '生産数', '不良率', '平均能率(本/時)']))
  for (const row of extras.weldRankOff) {
    lines.push(
      csvLine([
        row.rank,
        row.inspector_name ?? '',
  ...
  lines.push(csvLine(['【溶接工程あり — 検査員平均能率ランキング】']))
  lines.push(csvLine(['順位', '検査員', '件数', '生産数', '不良率', '平均能率(本/時)']))
  for (const row of extras.weldRankOn) {
    lines.push(
      csvLine([
        row.rank,
        row.inspector_name ?? '',
  ...
  lines.push(csvLine(['CD', '製品名', 'TOP検査員', '能率(本/時)', '対象人数', '生産数']))
  for (const row of extras.productRankTop) {
    lines.push(
      csvLine([
        row.product_cd,
        row.product_name ?? '',
        row.top_inspector_name ?? '',
        fmtEfficiency(row.top_efficiency_per_hour),
        row.ranked_inspector_count ?? 0,
        row.sum_actual_qty ?? 0,
      ]),
    )
  }
...
function kpiCardHtml(card: InspectionProductivityKpiCardPrint): string {
...
function inspectorRowRankHtml(index: number): string {
  let cls = 'row-rank'
...
const PRINT_INSPECTOR_TABLE_MAX = 5
const PRINT_WELD_TABLE_MAX = 8

function buildInspectorPrintTable(
  rows: Array<InspectionProductivityInspectorRow & { avg_efficiency_per_hour?: number | null }>,
  maxRows = 0,
): string {
  ...
  const body = display
    .map(
      (row, index) => `<tr>
        <td class="center">${inspectorRowRankHtml(index)}</td>
        <td>${escHtml(row.inspector_name ?? '—')}</td>
        ...
        <td class="num"><span class="pill pill--inspector">${escHtml(fmtEfficiency(row.avg_efficiency_per_hour))}</span></td>
      </tr>`,
    )
    .join('')
  const more =
  ...
  return `<table class="data data--inspector data--compact">
    <thead>${tableHead(['#', '検査員', '件', '生産', '不良率', '平均能率'])}</thead>
    <tbody>${body}</tbody>
  </table>${more}`
}

function buildProductPrintTable(
  rows: Array<InspectionProductivityProductRow & { avg_efficiency_per_hour?: number | null }>,
): string {
  if (!rows.length) return '<p class="empty">データなし</p>'
  const body = rows
    .map(
...
function buildWeldRankPrintTable(
  rows: InspectionProductivityWeldRankRow[],
  theme: 'off' | 'on',
  maxRows = 0,
): string {
  if (!rows.length) return '<p class="empty">データなし</p>'
  const pillClass = theme === 'on' ? 'pill--weld-on' : 'pill--weld-off'
  const display = maxRows > 0 ? rows.slice(0, maxRows) : rows
  const body = display
    .map(
      (row) => `<tr>
        <td class="center">${rankBadgeHtml(row.rank)}</td>
        <td>${escHtml(row.inspector_name ?? '—')}</td>
        <td class="num">${escHtml(fmtInt(row.session_count))}</td>
        <td class="num">${escHtml(fmtInt(row.sum_actual_qty))}</td>
        <td class="num warn">${escHtml(fmtPct(row.defect_rate_percent))}</td>
        <td class="num"><span class="pill ${pillClass}">${escHtml(fmtEfficiency(row.avg_efficiency_per_hour))}</span></td>
      </tr>`,
    )
    .join('')
  const more =
    maxRows > 0 && rows.length > maxRows
      ? `<p class="table-more">… 他 ${rows.length - maxRows} 名</p>`
      : ''
  return `<table class="data data--weld data--weld-${theme} data--compact">
    <thead>${tableHead(['順位', '検査員', '件', '生産', '不良率', '平均能率'])}</thead>
    <tbody>${body}</tbody>
  </table>${more}`
}

function buildProductRankInspectorTable(rows: InspectionProductivityInspectorRow[]): string {
  if (!rows.length) return '<p class="empty">データなし</p>'
  const body = rows
    .map(
      (row) => `<tr>
...
function buildProductRankOverviewTable(rows: InspectionProductivityProductInspectorRanking[]): string {
  if (!rows.length) return '<p class="empty">データなし</p>'
  const body = rows
    .map(
      (row) => `<tr>
        ...
        <td>${escHtml(row.product_name ?? '')}</td>
        <td>${escHtml(row.top_inspector_name ?? '—')}</td>
        <td class="num"><span class="pill pill--rank">${escHtml(fmtEfficiency(row.top_efficiency_per_hour))}</span></td>
        <td class="num">${escHtml(row.ranked_inspector_count ?? 0)}</td>
        <td class="num">${escHtml(fmtInt(row.sum_actual_qty))}</td>
      </tr>`,
    )
    .join('')
  return `<table class="data data--rank-overview">
    <thead>${tableHead(['CD', '製品名', 'TOP検査員', '能率', '対象人数', '生産'])}</thead>
    <tbody>${body}</tbody>
  </table>`
}

function buildProductRankHeroHtml(
  ranking: InspectionProductivityProductInspectorRanking,
  stats: InspectionProductivityProductRankPrintContext['stats'],
): string {
  return `<div class="rank-hero">
    ...
    <div class="rank-hero__stats">
      ...
      <div class="rank-hero__stat"><span class="rank-hero__stat-label">検査員</span><span class="rank-hero__stat-val">${escHtml(stats?.inspectorCount ?? ranking.ranked_inspector_count ?? 0)}<small>名</small></span></div>
      <div class="rank-hero__stat rank-hero__stat--accent"><span class="rank-hero__stat-label">TOP能率</span><span class="rank-hero__stat-val">${escHtml(fmtEfficiency(stats?.topEfficiency ?? ranking.top_efficiency_per_hour))}<small>本/時</small></span></div>
      <div class="rank-hero__stat"><span class="rank-hero__stat-label">平均能率</span><span class="rank-hero__stat-val">${escHtml(fmtEfficiency(stats?.avgEfficiency ?? null))}<small>本/時</small></span></div>
    </div>
  </div>`
}
...
function getPrintStyles(mode: 'full' | 'section', orientation: PrintOrientation = 'portrait'): string {
  ...
  return `
    ...
    * { box-sizing: border-box; }
    ...
    .pill--inspector { color: #4338ca; background: rgba(99,102,241,.1); border: 1px solid rgba(99,102,241,.15); }
...
<head>
  <meta charset="UTF-8" />
  <title>検査工程 — 生産性分析${options.sectionTitle ? ` — ${options.sectionTitle}` : ''}</title>
...
function buildFullPrintBody(ctx: InspectionProductivityReportContext): string {
  const {
    charts,
    inspectorRows,
    productRows,
    inspectorSectionAvgEfficiency,
    productSectionTotalQty,
    weldRankOff,
    weldRankOn,
  } = ctx

  const inspectorBadges = [
    panelBadge(`${inspectorRows.length} 名`, 'soft'),
    inspectorSectionAvgEfficiency != null
      ? panelBadge(`平均能率 ${fmtEfficiency(inspectorSectionAvgEfficiency)} 本/時`, 'inspector')
      : '',
  ].join('')
  ...
  ${panelSection('検査員別', { theme: 'inspector', badges: inspectorBadges, chartSrc: charts.inspector, chartAlt: '検査員別', tableHtml: buildInspectorPrintTable(inspectorRows, PRINT_INSPECTOR_TABLE_MAX) })}
  <div class="weld-split">
    ${panelSection('検査員平均能率ランキング', { subtitle: '溶接工程なし製品', theme: 'weld-off', badges: panelBadge(`${weldRankOff.length} 名`, 'weld-off'), tableHtml: buildWeldRankPrintTable(weldRankOff, 'off', PRINT_WELD_TABLE_MAX) })}
    ${panelSection('検査員平均能率ランキング', { subtitle: '溶接工程あり製品', theme: 'weld-on', badges: panelBadge(`${weldRankOn.length} 名`, 'weld-on'), tableHtml: buildWeldRankPrintTable(weldRankOn, 'on', PRINT_WELD_TABLE_MAX) })}
  </div>
...
export function buildInspectionProductivityPrintHtml(ctx: InspectionProductivityReportContext): string {
...
export function buildInspectionProductivitySectionPrintHtml(
  section: InspectionProductivityPrintSection,
  ctx: InspectionProductivityReportContext,
): string {
  const {
    charts,
    inspectorRows,
  ...
  if (section === 'full') {
    return buildInspectionProductivityPrintHtml(ctx)
  }
  ...
  if (section === 'inspector') {
    ...
    return buildPrintDocumentShell(ctx.filters, {
      sectionTitle: '検査員別',
      mode: 'section',
      body: panelSection('検査員別', {
        theme: 'inspector',
        badges,
        chartSrc: charts.inspector,
        chartAlt: '検査員別',
        tableHtml: buildInspectorPrintTable(inspectorRows),
      }),
    })
  }
  ...
  if (section === 'weld-rank') {
    return buildPrintDocumentShell(ctx.filters, {
      sectionTitle: '検査員平均能率ランキング',
      mode: 'section',
      body: `<div class="weld-split weld-split--section">
        ${panelSection('検査員平均能率ランキング', { subtitle: '溶接工程なし製品', theme: 'weld-off', badges: panelBadge(`${weldRankOff.length} 名`, 'weld-off'), tableHtml: buildWeldRankPrintTable(weldRankOff, 'off') })}
        ${panelSection('検査員平均能率ランキング', { subtitle: '溶接工程あり製品', theme: 'weld-on', badges: panelBadge(`${weldRankOn.length} 名`, 'weld-on'), tableHtml: buildWeldRankPrintTable(weldRankOn, 'on') })}
      </div>`,
    })
  }
  ...
  if (section === 'product-rank') {
    ...
    const selectedBody = selected
      ? `${buildProductRankHeroHtml(selected, productRank.stats)}
        ${panelSection('検査員別能率', { theme: 'rank', badges: panelBadge(`${selected.inspectors.length} 名`, 'rank'), chartSrc: charts.productRank ?? null, chartAlt: '検査員別能率', tableHtml: buildProductRankInspectorTable(selected.inspectors) })}`
...
export interface InspectionProductivityDailyBatchItem {
...
function buildDailyBatchPrintPageHeader(
  ...
  return `<header class="hd">
    <div class="hd__title">検査工程 — 生産性分析</div>
    <div class="hd__section">日別推移（検査員別）</div>
    ${buildMetaLineHtml(filters, printedAt, { compact: true })}
  </header>`
}

export function buildInspectionProductivityDailyBatchPrintHtml(
  filters: InspectionProductivityReportFilters,
  ...
  const body = items
    .map((item, idx) => {
      ...
      const panel = panelSection('日別推移', {
        ...
        chartAlt: `日別推移 — ${item.inspectorLabel}`,
        chartTall: true,
        tableHtml: '',
        flowBreak: true,
      })
...
<head>
  <meta charset="UTF-8" />
  <title>検査工程 — 生産性分析 — 日別推移（検査員別）</title>
...
export function printInspectionProductivityDailyBatch(
  filters: InspectionProductivityReportFilters,
  items: InspectionProductivityDailyBatchItem[],
) {
...
export function printInspectionProductivitySection(
  ...
  openPrintDocument(buildInspectionProductivitySectionPrintHtml(section, ctx))
}
...
export function exportInspectionSessionsCsv(
  data: InspectionProductivityAnalysisData,
  filters: InspectionProductivityReportFilters,
) {
...
export function exportInspectionSummaryCsv(
  data: InspectionProductivityAnalysisData,
  filters: InspectionProductivityReportFilters,
...
export function printInspectionProductivityReport(payload: InspectionProductivityPrintPayload) {
  openPrintDocument(buildInspectionProductivityPrintHtml(payload))
}
