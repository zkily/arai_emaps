...
班長が **検査モニタ** 画面で、稼働中の検査員に対して「次に検査する製品」を指定した場合、ツールバー直下（検査生産中ストリップの上）に **「次の製品（指定）」** の読み取り専用バーが表示されます。
...
- 表示のみです。生産終了後も **検査製品は手動で選択** してください（自動切替はしません）。
- 約 5 秒ごとにサーバーと同期し、指定の更新・解除が反映されます。

**班長側（検査モニタ）**

1. メニュー **MES** → **モニタリング** → **検査モニタ** を開く
2. 稼働中の検査員カードで **次製品指定** を押す
3. 次製品を選択して **保存**（解除は **指定解除**）
...
- 班長が **次製品（指定）** を出している場合は、ストリップを参考に製品を選んでください（自動選択はされません）。
