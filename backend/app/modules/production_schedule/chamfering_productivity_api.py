# coding: utf-8
"""面取工程 生産性分析 API（chamfering_production_indicator 集計）"""
...
def _chamfering_row_work_sec(item: dict[str, Any]) -> int:
...
def _chamfering_row_paused_sec(item: dict[str, Any]) -> int:
    shift_h = _to_float(item.get("shift_hours"))
    work_h = _to_float(item.get("work_hours"))
    if shift_h > work_h > 0:
        return max(0, int(round((shift_h - work_h) * 3600)))
    pause_h = sum(_to_float(item.get(field)) for _, field in CHAMFERING_LINE_METRICS_LOSS_FIELDS)
...
def _normalize_chamfering_row(row: dict[str, Any]) -> dict[str, Any]:
    item = dict(row)
...
def _new_line_metrics_bucket(*, line_key: str, line_name: str) -> dict[str, Any]:
    ...
    for header, _field in CHAMFERING_LINE_METRICS_LOSS_FIELDS:
        bucket["defects"][header] = 0
    bucket["defects"]["面取不良"] = 0
...
def register_routes(router: APIRouter) -> None:
    @router.get("/plan/chamfering-production-indicator/lines")
    async def get_chamfering_production_indicator_lines(
        ...
        try:
            ...
            if "chamfering_production_indicator" in msg and (
                ...
                raise HTTPException(
                    status_code=503,
                    detail="chamfering_production_indicator テーブルが存在しません。backend/database/migrations/72_chamfering_production_indicator.sql を実行してください",
                ) from e
            raise HTTPException(status_code=500, detail=str(e)) from e
        return {"success": True, "data": [{"line_name": name} for name in lines]}

    @router.get("/plan/chamfering-production-indicator/productivity-analysis")
    async def get_chamfering_productivity_analysis(
        start_date: str = Query(..., description="開始日 YYYY-MM-DD"),
        end_date: str = Query(..., description="終了日 YYYY-MM-DD"),
        production_line: Optional[str] = Query(None, description="ライン名"),
        product_cd: Optional[str] = Query(None, description="製品CD"),
        include_incomplete: bool = Query(False, description="互換用（面取指標は常に実績行）"),
        limit: int = Query(10000, ge=1, le=20000),
        db: AsyncSession = Depends(get_db),
        current_user: User = Depends(verify_token_and_get_user),
    ):
        """面取能率分析（chamfering_production_indicator 集計）"""
        start_d = _parse_date_ymd(start_date)
        end_d = _parse_date_ymd(end_date)
        ...
        sum_chamfer_defect_qty = 0
        sum_sw_defect_qty = 0

        for row in rows:
            item = _normalize_chamfering_row(dict(row))
            ...
            sum_chamfer_defect_qty += int(item.get("chamfer_defect_quantity") or 0)
            sum_sw_defect_qty += int(item.get("sw_defect_quantity") or 0)
            net_sec = _chamfering_row_work_sec(item)
            paused_sec = _chamfering_row_paused_sec(item)
            ...
            stop_h = sum(_to_float(item.get(field)) for _, field in CHAMFERING_LINE_METRICS_LOSS_FIELDS)
            ...
            for header, field in CHAMFERING_LINE_METRICS_LOSS_FIELDS:
        ...
        return {
            ...
            "data": {
                ...
                "defect_by_item": [
                    *(
                        [{"defect_cd": "面取不良", "qty": sum_chamfer_defect_qty}]
                        if sum_chamfer_defect_qty > 0
                        else []
                    ),
                    *(
                        [{"defect_cd": "SW不良", "qty": sum_sw_defect_qty}]
                        if sum_sw_defect_qty > 0
                        else []
                    ),
                ],
