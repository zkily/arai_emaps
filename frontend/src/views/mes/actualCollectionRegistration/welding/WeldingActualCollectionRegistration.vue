...
const totalDefects = computed(() =>
  Object.values(form.value.defects).reduce((s, n) => s + Math.max(0, Math.round(Number(n) || 0)), 0),
)

/** ③製品名選択後のみ ④〜⑦ を入力可能（編集時は製品確定済み） */
const operatorSelected = computed(() => {
  const id = form.value.operatorUserId
  return id != null && Number.isFinite(Number(id)) && Number(id) > 0
})

const machineSelected = computed(() => Boolean(form.value.weldingMachineId))

const productSelected = computed(
  () => isEdit.value || (machineSelected.value && operatorSelected.value && Boolean(form.value.productCd?.trim())),
)
...
const defectInputRefs = new Map<string, InstanceType<typeof ElInput>>()
...
function focusDefectById(defectCd: string | null | undefined): void {
  if (!defectCd) return
  nextTick(() => {
    const comp = defectInputRefs.get(defectCd)
    comp?.focus()
...
function setDefectInputRef(defectCd: string) {
  return (el: Element | ComponentPublicInstance | null) => {
    if (el) defectInputRefs.set(defectCd, el as InstanceType<typeof ElInput>)
    else defectInputRefs.delete(defectCd)
  }
}

function onDefectCardClick(defectCd: string, event: MouseEvent): void {
  if (!productSelected.value) return
  const target = event.target as HTMLElement
  if (target.closest('.iar-defect__btn, .el-button')) return
  focusDefectById(defectCd)
}

function onDefectQtyFocus(event: FocusEvent): void {
...
function onStopMinEnter(e: Event): void {
  if (!(e instanceof KeyboardEvent) || e.key !== 'Enter') return
  e.preventDefault()
  focusDefectById(firstDefectId.value)
}

function defectQtyDisplay(defectCd: string): string {
  const n = defectCount(defectCd)
  return n > 0 ? String(n) : ''
}
    ...
    <header class="iar-hero iar-rise">
      <div class="iar-hero__main">
        ...
        <div>
          ...
          <h1 class="iar-hero__title">溶接実績収集登録</h1>
        </div>
      </div>
      <div class="iar-hero__chips">
        <span class="iar-chip iar-chip--blue"><i>①</i>生産日</span>
        <span class="iar-chip iar-chip--violet"><i>②</i>溶接設備</span>
        <span class="iar-chip iar-chip--teal"><i>③</i>溶接作業者</span>
        <span class="iar-chip iar-chip--amber"><i>④</i>製品</span>
        <span class="iar-chip iar-chip--indigo"><i>⑤</i>生産数</span>
        <span class="iar-chip iar-chip--rose"><i>⑥</i>時間</span>
        <span class="iar-chip iar-chip--slate"><i>⑦</i>不良</span>
        <span class="iar-chip iar-chip--blue"><i>⑧</i>備考</span>
      </div>
    </header>
      ...
      <div class="iar-form">
        <div class="iar-form__row iar-form__row--triple">
          ...
          <div class="iar-field iar-field--c2">
            <label class="iar-field__label"><span class="iar-step iar-step--2">②</span>溶接設備</label>
            <el-select
              v-model="form.weldingMachineId"
              filterable
              clearable
              placeholder="選択"
          ...
          <div class="iar-field iar-field--c3">
            <label class="iar-field__label"><span class="iar-step iar-step--3">③</span>溶接作業者</label>
            <el-select
              ...
              :disabled="!machineSelected && !isEdit"
            >
              <template #prefix><el-icon><User /></el-icon></template>
              <el-option
                v-for="u in operators"
                :key="u.id"
                :label="operatorLabel(u.id)"
                :value="u.id"
              />
            </el-select>
          </div>
        ...
        <div class="iar-form__row iar-form__row--product-qty">
          <div class="iar-field iar-field--c4-product">
            ...
            <el-select
              ...
              :disabled="!machineSelected || !operatorSelected"
        ...
        <p v-if="!isEdit && !machineSelected" class="iar-form__lock-hint iar-form__lock-hint--pulse">
          ...
          ② 溶接設備を選択すると、③ 溶接作業者を入力できます
        </p>
        <p v-else-if="!isEdit && machineSelected && !operatorSelected" class="iar-form__lock-hint iar-form__lock-hint--pulse">
          <el-icon><Warning /></el-icon>
          ③ 溶接作業者を選択すると、④ 製品名を入力できます
        </p>
        ...
        <div class="iar-defects iar-field--c7">
          <label class="iar-field__label">
            <span class="iar-step iar-step--7">⑦</span>不良（項目別）
            <span v-if="totalDefects > 0" class="iar-defects__total">{{ totalDefects }}</span>
          </label>
          <div v-loading="loadingDefectItems" class="iar-defects__box">
            <template v-if="defectItemGroups.length">
              <section
                v-for="group in defectItemGroups"
                :key="group.processCd"
                class="iar-defects__group"
              >
                <header class="iar-defects__group-head">
                ...
                <div class="iar-defects__grid" :style="defectGridStyle(group)">
                  <div
                    ...
                    :key="item.id"
                    class="iar-defect"
                    :class="{ 'iar-defect--active': defectCount(item.id) > 0 }"
                    @click="onDefectCardClick(item.id, $event)"
                  >
                    <span class="iar-defect__name">{{ defectItemLabel(item.id, item.label) }}</span>
                    <div class="iar-defect__ctrl">
                      <el-button
                        circle
                        class="iar-defect__btn iar-defect__btn--minus"
                        :icon="Minus"
                        :disabled="!productSelected || defectCount(item.id) <= 0"
                        @click.stop="bumpDefect(item.id, -1)"
                      />
                      <el-input
                        :ref="setDefectInputRef(item.id)"
                        :model-value="defectQtyDisplay(item.id)"
                        :disabled="!productSelected"
                        inputmode="numeric"
                        placeholder="0"
                        class="iar-defect__qty-input"
                        @update:model-value="(v) => onDefectQtyInput(item.id, v)"
                        @focus="onDefectQtyFocus"
                        @click.stop
                      />
                      <el-button
                        circle
                        class="iar-defect__btn iar-defect__btn--plus"
                        :icon="Plus"
                        :disabled="!productSelected"
                        @click.stop="bumpDefect(item.id, 1)"
                      />
            ...
            <el-empty v-else :image-size="40" description="不良項目なし" />
          </div>
        </div>

        <div class="iar-remarks iar-field--c8 iar-form__footer">
          <div class="iar-remarks__row">
            <label class="iar-field__label iar-remarks__label">
              <span class="iar-step iar-step--8">⑧</span>備考
    ...
    <section class="iar-panel iar-panel--table iar-rise iar-rise--d2">
      ...
      <div class="iar-table-wrap">
        ...
        >
          ...
          <el-table-column label="溶接設備" width="88" align="left" header-align="left" show-overflow-tooltip>
            <template #default="{ row }">{{ row.welding_machine || '—' }}</template>
          </el-table-column>
          <el-table-column label="作業者" width="96" align="left" header-align="left" show-overflow-tooltip>
            <template #default="{ row }">{{ operatorLabel(row.mes_operator_user_id) }}</template>
          </el-table-column>
          ...
          <el-table-column label="生産数" width="72" align="right" header-align="right">
            <template #default="{ row }">
              <span class="iar-table__qty">{{ row.actual_production_quantity ?? '—' }}</span>
            </template>
          </el-table-column>
          <el-table-column label="不良" width="56" align="center" header-align="center">
            <template #default="{ row }">
              <span class="iar-table__defect" :class="{ 'iar-table__defect--on': (row.defect_qty ?? 0) > 0 }">
                {{ row.defect_qty ?? 0 }}
              </span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="開始" width="138" align="center" header-align="center">
            <template #default="{ row }">
              <span class="iar-table__time">{{ formatDateTime(row.mes_production_started_at) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="終了" width="138" align="center" header-align="center">
            <template #default="{ row }">
              <span class="iar-table__time">{{ formatDateTime(row.mes_production_ended_at) }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="manual_registration_note" label="備考" min-width="72" align="left" header-align="left" show-overflow-tooltip />
          <el-table-column
