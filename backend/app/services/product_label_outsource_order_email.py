...
EVENT_CODE = "PRODUCT_LABEL_OUTSOURCE_ORDER"
REFERENCE_PREFIX = "product_label_outsource_order"
...
async def fetch_outsource_label_configs(db: AsyncSession) -> list[dict]:
    query = (
        select(ProductLabelConfig, Product.product_name, Product.status, ProcessRoute.description)
        .outerjoin(Product, _product_cd_join())
        .outerjoin(ProcessRoute, Product.route_cd == ProcessRoute.route_cd)
        .where(ProductLabelConfig.supply_type == SUPPLY_TYPE_OUTSOURCE)
        .order_by(Product.product_name.asc(), ProductLabelConfig.product_cd.asc())
    )
    result = await db.execute(query)
    rows = result.all()
    items: list[dict] = []
    for config, master_name, product_status, route_desc_raw in rows:
...
def _build_item_table_html(items: list[dict]) -> str:
    lines = [
        ...
        "<thead><tr>"
        "<th>製品CD</th><th>加工用製品名</th><th>入数</th><th>用紙色</th><th>注文数</th>"
        "</tr></thead><tbody>",
    ]
    for row in items:
        qty = row.get("process_unit_qty")
        qty_text = "" if qty is None else str(qty)
        lines.append(
            "<tr>"
            f"<td>{html.escape(str(row.get('product_cd') or ''))}</td>"
            f"<td>{html.escape(str(row.get('label_product_name') or row.get('master_product_name') or ''))}</td>"
...
def _build_item_list_text(items: list[dict]) -> str:
    parts: list[str] = []
    for row in items:
        name = row.get("label_product_name") or row.get("master_product_name") or ""
        parts.append(
            f"- {row.get('product_cd')} / {name} / 入数:{row.get('process_unit_qty') or '—'} "
