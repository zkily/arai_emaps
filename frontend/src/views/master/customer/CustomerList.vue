  ...
  <div class="customer-master-container">
      ...
      <div class="header-content">
        <div class="title-row">
          ...
          <div class="stat-badges">
            <div class="stat-badge"><span class="stat-number">{{ customerList.length }}</span><span class="stat-label">{{ t('master.common.items') }}</span></div>
        ...
        <el-button type="primary" @click="openForm()" class="add-btn" size="small">➕ {{ t('master.customer.addCustomer') }}</el-button>
      </div>
    </div>
    ...
    <div class="search-section">
      <div class="search-row">
        <div class="search-group">
          <el-input v-model="filters.keyword" :placeholder="t('master.customer.searchPlaceholder')" clearable @input="handleFilter" class="search-input" size="small">
        ...
        <div class="filter-group">
          ...
          <el-select v-model="filters.customer_type" :placeholder="t('master.customer.type')" clearable @change="handleFilter" size="small" class="filter-select">
    ...
    <div class="table-section">
      <el-table :data="filteredList" stripe highlight-current-row v-loading="loading" class="modern-table"
        ...
        <el-table-column prop="customer_cd" :label="t('master.customer.customerCD')" width="100" align="center">
          <template #default="{ row }"><span class="code-cell">{{ row.customer_cd }}</span></template>
        </el-table-column>
        <el-table-column prop="customer_name" :label="t('master.customer.customerName')" min-width="140" show-overflow-tooltip>
          <template #default="{ row }"><span class="name-cell">{{ row.customer_name }}</span></template>
        </el-table-column>
        <el-table-column prop="phone" :label="t('master.customer.phone')" width="130" show-overflow-tooltip />
        <el-table-column prop="address" :label="t('master.customer.address')" min-width="180" show-overflow-tooltip />
        <el-table-column prop="customer_type" :label="t('master.customer.type')" width="80" align="center">
          <template #default="{ row }">
            <el-tag :type="getCustomerTypeTagType(row.customer_type)" size="small" effect="plain">{{ row.customer_type || '—' }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column :label="t('master.common.actions')" fixed="right" width="110" align="center">
          <template #default="{ row }">
            <div class="action-buttons">
              ...
              <el-button size="small" type="danger" plain @click="deleteCustomer(row.id)" class="action-btn">🗑️</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <div class="footer-section">
      <div class="result-info"><el-icon><DataAnalysis /></el-icon><span>{{ t('master.common.displayCount', { shown: filteredList.length, total: customerList.length }) }}</span></div>
    </div>
...
import { getCustomerList, deleteCustomerById, updateCustomerStatus } from '@/api/master/customerMaster'
...
const handleFilter = () => {}
const clearFilters = () => { filters.value = { keyword: '', status: '', customer_type: '' }; fetchList() }

const activeCount = computed(() => customerList.value.filter((row) => row.status === 1).length)

function getCustomerTypeTagType(type: string | undefined): 'primary' | 'success' | 'warning' | 'info' {
  if (!type) return 'info'
...
const filteredList = computed(() => {
  ...
  if (filters.value.customer_type) result = result.filter((row) => row.customer_type === filters.value.customer_type)
  return result
})
...
async function deleteCustomer(id: number | undefined) {
  if (id == null) return
  try {
    await ElMessageBox.confirm(t('master.customer.confirmDelete'), t('common.confirm'), { type: 'warning' })
...
async function fetchList() {
  ...
  try {
    ...
    customerList.value = (res.list ?? res.data?.list ?? []).map((row) => ({ ...row, statusLoading: false }))
  } catch (e) {
    console.error(e)
  } finally {
    loading.value = false
...
<style scoped>
.customer-master-container { padding: 12px 16px; background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%); min-height: 100vh; }
...
@media (max-width: 768px) {
  .customer-master-container { padding: 8px; }
