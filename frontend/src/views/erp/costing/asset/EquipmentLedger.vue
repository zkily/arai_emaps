      ...
      <el-table :data="assetList" v-loading="loading" stripe border>
        ...
        <el-table-column prop="type" label="種別" width="100" align="center">
          <template #default="{ row }">
            <el-tag size="small">
              {{ assetTypeLabel(String(row.type)) }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="status" label="ステータス" width="90" align="center">
          <template #default="{ row }">
            <el-tag
              ...
              "
            >
              {{ assetStatusLabel(String(row.status)) }}
            </el-tag>
          </template>
        </el-table-column>
...
const assetTypeLabel = (t: string) => ASSET_TYPE_LABELS[t] ?? t
...
const assetStatusLabel = (s: string) => ASSET_STATUS_LABELS[s] ?? s
...
const handleCreate = () => {
  ElMessage.info('新規設備登録画面を開きます')
}
const handleExport = () => {
  ElMessage.info('設備台帳をエクスポートします')
}
const handleView = (row: any) => {
  ElMessage.info(`設備 ${row.asset_no} の詳細`)
}
</script>
