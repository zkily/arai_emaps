# coding: utf-8
"""生産管理指標(YYYY年度-成形) CSV/Excel → forming_production_indicator 取込（CLI・ファイル監視共用）"""
...
EXTERNAL_SYNC_KEY_PREFIX = "form_excel:"
...
FORMING_SHEET_HEADER_MARKERS = ("日付", "ライン", "品名", "生産数")
...
@dataclass
class ParsedFormingIndicatorRow:
    source_line: int
    ...
    waiting_repair_hours: float | None
    ...
    setup_adjustment_flag: str | None
    yellow_box_qty: int | None
    metric_60: str | None
...
def parse_logical_row(
    ...
    *,
    ...
    return ParsedFormingIndicatorRow(
        ...
        waiting_repair_hours=fields.get("waiting_repair_hours"),
        ...
        setup_adjustment_flag=fields.get("setup_adjustment_flag"),
        yellow_box_qty=fields.get("yellow_box_qty"),
...
def _sheet_has_forming_headers(sheet) -> bool:
    ...
    return all(marker in headers for marker in FORMING_SHEET_HEADER_MARKERS)


def resolve_forming_data_sheet(wb, *, filepath: str = ""):
    for name in wb.sheetnames:
        sheet = wb[name]
        if _sheet_has_forming_headers(sheet):
            return name, sheet
    active = wb.active
    if active is not None and _sheet_has_forming_headers(active):
        return active.title, active
    available = ", ".join(wb.sheetnames)
    location = f"（{filepath}）" if filepath else ""
    raise ValueError(
        f"成形生産管理指標のデータシート（日付/ライン/品名/生産数 ヘッダ）が見つかりません"
        f"{location}。利用可能: {available}"
    )


def iter_forming_excel_rows(filepath: str) -> Iterator[tuple[int, dict[str, Any]]]:
    import openpyxl
    ...
    try:
        _resolved_name, sheet = resolve_forming_data_sheet(wb, filepath=filepath)
...
def parse_rows_from_source(
    ...
    *,
...
) -> tuple[list[ParsedFormingIndicatorRow], list[str], int]:
    path = Path(source_path)
    source_label = path.name
    fiscal_year = fiscal_year_from_source(source_label)
    parsed: list[ParsedFormingIndicatorRow] = []
    errors: list[str] = []
    skipped = 0
    ...
    if suffix == ".csv":
        ...
        row_iter = iter_forming_excel_rows(str(path))
    else:
...
def _row_to_insert_tuple(row: ParsedFormingIndicatorRow) -> tuple:
    return (
        row.fiscal_year,
...
def sync_parsed_rows_to_db(
    cursor,
    rows: list[ParsedFormingIndicatorRow],
    *,
    source_file: str,
    dry_run: bool = False,
) -> ImportSyncResult:
    ...
    cursor.execute(
        "SELECT COUNT(1) FROM forming_production_indicator WHERE source_file = %s",
        (source_file,),
    )
    result.deleted = int(cursor.fetchone()[0] or 0)
    if result.deleted:
        cursor.execute(
            "DELETE FROM forming_production_indicator WHERE source_file = %s",
            (source_file,),
        )
    ...
    sql = f"INSERT INTO forming_production_indicator ({', '.join(INSERT_COLUMNS)}) VALUES ({placeholders})"
...
def sync_forming_source_file(
    ...
    *,
    ...
    """CSV/Excel 1 ファイルを forming_production_indicator に全件置換同期"""
    ...
    try:
        ...
        if not dry_run:
            conn.commit()
        logger.info(
            "forming_production_indicator 同期完了: parsed=%s inserted=%s deleted=%s path=%s",
            result.parsed,
            result.inserted,
            result.deleted,
            source_path,
