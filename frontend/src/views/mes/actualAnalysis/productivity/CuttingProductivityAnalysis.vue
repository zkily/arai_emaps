...
import {
  fetchCuttingProductivityAnalysis,
  type CuttingProductivityAnalysisData,
  type CuttingProductivityBucket,
  type CuttingProductivityDailyRow,
  type CuttingProductivityOperatorRow,
  type CuttingProductivityProductOperatorRanking,
  type CuttingProductivityProductRow,
  type CuttingProductivitySessionRow,
} from '@/api/cuttingProductionIndicator'
import type { CuttingProductionIndicatorLineOption as CuttingLineOption } from '@/api/cuttingProductionIndicator'
...
import {
  printCuttingProductivityDailyBatch,
  printCuttingProductivityOperatorProductBatch,
  printCuttingProductivityReport,
  printCuttingProductivitySection,
  type CuttingProductivityDailyBatchItem,
  type CuttingProductivityOperatorProductBatchItem,
  type CuttingProductivityPrintSection,
  type CuttingProductivityReportContext,
  type CuttingProductivityReportFilters,
} from './cuttingProductivityReport'

defineOptions({ name: 'MesCuttingProductivityAnalysis' })
...
const analysisData = ref<CuttingProductivityAnalysisData | null>(null)
const lineOptions = ref<CuttingLineOption[]>([])
...
const emptySummary = (): CuttingProductivityBucket => ({
  session_count: 0,
...
const productRankList = computed((): CuttingProductivityProductOperatorRanking[] => {
...
function buildProductOperatorRankingFromSessions(
  sessions: CuttingProductivityAnalysisData['sessions'],
  ...
  const productMap = new Map<
    string,
    {
      ...
      operators: Map<string, CuttingProductivityOperatorRow>
    }
  >()
  ...
  const result: CuttingProductivityProductOperatorRanking[] = []
  for (const prod of productMap.values()) {
    const operators: CuttingProductivityOperatorRow[] = []
...
const podiumOperators = computed(() => {
  ...
  return order
    ...
    .filter((r): r is CuttingProductivityOperatorRow => r != null)
})
...
function productRankOptionLabel(p: CuttingProductivityProductOperatorRanking): string {
...
function sortByEfficiency(a: CuttingProductivityOperatorRow, b: CuttingProductivityOperatorRow): number {
...
type OperatorDisplayRow = CuttingProductivityOperatorRow & {
  avg_efficiency_per_hour: number | null
}

type ProductDisplayRow = CuttingProductivityProductRow & {
  avg_efficiency_per_hour: number | null
}
...
function sortByOperatorDefectRate(a: CuttingProductivityOperatorRow, b: CuttingProductivityOperatorRow): number {
...
function rankOverviewRowClass({ row }: { row: CuttingProductivityProductOperatorRanking }): string {
...
function buildReportFilters(): CuttingProductivityReportFilters | null {
...
function buildReportContext(): CuttingProductivityReportContext | null {
...
async function handleReportCommand(command: string | number | object) {
  ...
  try {
    ...
    if (cmd === 'print-full') {
      printCuttingProductivityReport(ctx)
      return
    }

    const sectionMap: Record<string, CuttingProductivityPrintSection> = {
      'print-daily': 'daily',
    ...
    if (section) {
      printCuttingProductivitySection(section, ctx)
      return
    }
  } catch (err) {
...
async function captureDailyChartDataUrl(daily: CuttingProductivityDailyRow[]): Promise<string | null> {
...
async function handleBatchDailyByOperatorPrint() {
  ...
  printCuttingProductivityDailyBatch(filters, items)
}
...
async function handleBatchOperatorProductPrint() {
  ...
  printCuttingProductivityOperatorProductBatch(filters, items)
}
...
async function loadProductOptions() {
  ...
  try {
    ...
    productOptions.value = filterCuttingSelectableProducts(list)
  } catch {
