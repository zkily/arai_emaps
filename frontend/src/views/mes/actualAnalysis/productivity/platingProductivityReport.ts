import type {
  PlatingProductivityAnalysisData,
  PlatingProductivityOperatorRow,
  PlatingProductivityProductOperatorRanking,
  PlatingProductivityProductRow,
  PlatingProductivitySessionRow,
} from '@/api/platingProductionIndicator'

export interface PlatingProductivityReportFilters {
  startDate: string
  endDate: string
  lineLabel: string
  productLabel: string
...
export type PlatingProductivityKpiTone = 'indigo' | 'sky' | 'amber' | 'emerald' | 'violet'

export interface PlatingProductivityKpiCardPrint {
  label: string
  value: string
  hint: string
  tone: PlatingProductivityKpiTone
}

export interface PlatingProductivityPrintCharts {
  daily: string | null
  operator: string | null
  product: string | null
}

export interface PlatingProductivityPrintChartsExtended extends PlatingProductivityPrintCharts {
  productRank?: string | null
}

export interface PlatingProductivityProductRankPrintContext {
  selected: PlatingProductivityProductOperatorRanking | null
  topOverview: PlatingProductivityProductOperatorRanking[]
  stats: {
...
export interface PlatingProductivityReportContext {
  filters: PlatingProductivityReportFilters
  kpiCards: PlatingProductivityKpiCardPrint[]
  charts: PlatingProductivityPrintChartsExtended
  lineRows: Array<PlatingProductivityOperatorRow & { avg_efficiency_per_hour?: number | null }>
  productRows: Array<PlatingProductivityProductRow & { avg_efficiency_per_hour?: number | null }>
  lineSectionAvgEfficiency: number | null
  productSectionTotalQty: number
  productRank: PlatingProductivityProductRankPrintContext
}

export type PlatingProductivityPrintSection =
  | 'full'
  | 'daily'
  | 'operator'
  | 'product'
...
/** @deprecated use PlatingProductivityReportContext */
export type PlatingProductivityPrintPayload = PlatingProductivityReportContext
...
function reportFileBase(filters: PlatingProductivityReportFilters): string {
  return `溶接生産性分析_${filters.startDate}_${filters.endDate}`
}

function metaCsvLines(filters: PlatingProductivityReportFilters): string[] {
  return [
    csvLine(['# 溶接工程 — 生産性分析']),
...
export function buildSummaryCsv(
  ...
  extras: {
    lineRows: Array<PlatingProductivityOperatorRow & { avg_efficiency_per_hour?: number | null }>
    productRows: Array<PlatingProductivityProductRow & { avg_efficiency_per_hour?: number | null }>
    productRankTop: PlatingProductivityProductOperatorRanking[]
...
function kpiCardHtml(card: PlatingProductivityKpiCardPrint): string {
...
function buildOperatorPrintTable(
  rows: Array<PlatingProductivityOperatorRow & { avg_efficiency_per_hour?: number | null }>,
  maxRows = 0,
): string {
...
function buildProductPrintTable(
  rows: Array<PlatingProductivityProductRow & { avg_efficiency_per_hour?: number | null }>,
): string {
  if (!rows.length) return '<p class="empty">データなし</p>'
  const body = rows
    .map(
...
function buildProductRankOperatorTable(rows: PlatingProductivityOperatorRow[]): string {
  if (!rows.length) return '<p class="empty">データなし</p>'
  const body = rows
    .map(
      (row) => `<tr>
...
function buildProductRankOverviewTable(rows: PlatingProductivityProductOperatorRanking[]): string {
  if (!rows.length) return '<p class="empty">データなし</p>'
  const body = rows
    .map(
      (row) => `<tr>
...
function buildProductRankHeroHtml(
  ranking: PlatingProductivityProductOperatorRanking,
  stats: PlatingProductivityProductRankPrintContext['stats'],
): string {
...
function buildFullPrintBody(ctx: PlatingProductivityReportContext): string {
  const {
    charts,
    lineRows,
    productRows,
...
export function buildPlatingProductivityPrintHtml(ctx: PlatingProductivityReportContext): string {
...
export function buildPlatingProductivitySectionPrintHtml(
  section: PlatingProductivityPrintSection,
  ctx: PlatingProductivityReportContext,
): string {
  const {
    charts,
    lineRows,
  ...
  if (section === 'full') {
    return buildPlatingProductivityPrintHtml(ctx)
  }
...
export interface PlatingProductivityDailyBatchItem {
...
export function buildPlatingProductivityDailyBatchPrintHtml(
  filters: PlatingProductivityReportFilters,
  items: PlatingProductivityDailyBatchItem[],
): string {
  const printedAt = printedAtJa()
  const body = items
    .map((item, idx) => {
...
export function printPlatingProductivityDailyBatch(
  filters: PlatingProductivityReportFilters,
  items: PlatingProductivityDailyBatchItem[],
) {
  openPrintDocument(buildPlatingProductivityDailyBatchPrintHtml(filters, items))
}

export interface PlatingProductivityOperatorProductBatchItem {
  lineLabel: string
  productCount: number
  sessionCount: number
  sumActualQty: number
...
export function buildPlatingProductivityOperatorProductBatchPrintHtml(
  filters: PlatingProductivityReportFilters,
  items: PlatingProductivityOperatorProductBatchItem[],
): string {
  const printedAt = printedAtJa()
  const body = items
    .map((item, idx) => {
...
export function printPlatingProductivityOperatorProductBatch(
  filters: PlatingProductivityReportFilters,
  items: PlatingProductivityOperatorProductBatchItem[],
) {
  openPrintDocument(buildPlatingProductivityOperatorProductBatchPrintHtml(filters, items))
}

export function printPlatingProductivityReport(payload: PlatingProductivityPrintPayload) {
  openPrintDocument(buildPlatingProductivityPrintHtml(payload))
}

export function printPlatingProductivitySection(
  section: PlatingProductivityPrintSection,
  ctx: PlatingProductivityReportContext,
) {
  openPrintDocument(buildPlatingProductivitySectionPrintHtml(section, ctx))
}
