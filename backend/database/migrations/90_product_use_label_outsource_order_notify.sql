...
VALUES
('PRODUCT_USE_LABEL_OUTSOURCE_ORDER', '製品用ラベル 外注注文', '外注区分の製品用ラベル注文をメール送信（注文一覧＋ラベルPDF添付）', 0, 1, 0, 0, 1)
...
VALUES
(
  'PRODUCT_USE_LABEL_OUTSOURCE_ORDER',
  '製品用ラベル 外注注文',
  '【Smart-EMAP】製品用ラベル外注注文 {item_count}件',
  '<p>製品用ラベルの外注注文があります。</p><p>注文件数: <strong>{item_count}</strong> 件<br>注文数合計: <strong>{total_order_qty}</strong><br>送信者: {sent_by}<br>送信日時: {sent_at}</p>{item_table}<p>添付: ラベルPDF {attachment_count} 枚（{attachment_names}）</p><p>Smart-EMAP マスタ管理</p>',
  'PRODUCT_USE_LABEL_OUTSOURCE_ORDER',
  'ja',
