    ...
    <header class="ipa-hero ipa-fade-in">
      <div class="ipa-hero__main">
        ...
        <div class="ipa-hero__text">
          ...
          <h1 class="ipa-hero__title">成形工程 — 生産性分析</h1>
...
import {
  fetchFormingProductivityAnalysis,
  type FormingProductivityAnalysisData,
  type FormingProductivityBucket,
  type FormingProductivityDailyRow,
  type FormingProductivityOperatorRow,
  type FormingProductivityProductOperatorRanking,
  type FormingProductivityProductRow,
  type FormingProductivitySessionRow,
} from '@/api/formingProductionIndicator'
import type { FormingProductionIndicatorLineOption as FormingLineOption } from '@/api/formingProductionIndicator'
import { getProductList } from '@/api/master/productMaster'
import type { Product } from '@/types/master'
import { getJSTToday, parseDateAsJST } from '@/utils/dateFormat'
import { lineLabel, loadFormingLineOptions, resolveFormingMetricsLabel } from '@/views/mes/shared/formingLineFilter'
import { filterFormingSelectableProducts } from '@/views/mes/shared/formingProductFilter'
import {
  CHART_INIT_OPTS,
  chartTheme,
...
import {
  printFormingProductivityDailyBatch,
  printFormingProductivityOperatorProductBatch,
  printFormingProductivityReport,
  printFormingProductivitySection,
  type FormingProductivityDailyBatchItem,
  type FormingProductivityOperatorProductBatchItem,
  type FormingProductivityPrintSection,
  type FormingProductivityReportContext,
  type FormingProductivityReportFilters,
} from './formingProductivityReport'

defineOptions({ name: 'MesFormingProductivityAnalysis' })
...
const analysisData = ref<FormingProductivityAnalysisData | null>(null)
const lineOptions = ref<FormingLineOption[]>([])
...
const emptySummary = (): FormingProductivityBucket => ({
  session_count: 0,
...
const productRankList = computed((): FormingProductivityProductOperatorRanking[] => {
...
function buildProductOperatorRankingFromSessions(
  sessions: FormingProductivityAnalysisData['sessions'],
  ...
  const productMap = new Map<
    string,
    {
      ...
      operators: Map<string, FormingProductivityOperatorRow>
    }
  >()
  ...
  const result: FormingProductivityProductOperatorRanking[] = []
  for (const prod of productMap.values()) {
    const operators: FormingProductivityOperatorRow[] = []
...
const podiumOperators = computed(() => {
  ...
  return order
    ...
    .filter((r): r is FormingProductivityOperatorRow => r != null)
})
...
function productRankOptionLabel(p: FormingProductivityProductOperatorRanking): string {
...
function sortByEfficiency(a: FormingProductivityOperatorRow, b: FormingProductivityOperatorRow): number {
...
type OperatorDisplayRow = FormingProductivityOperatorRow & {
  avg_efficiency_per_hour: number | null
}

type ProductDisplayRow = FormingProductivityProductRow & {
  avg_efficiency_per_hour: number | null
}

/** 期間内：生産合計 ÷ 正味稼働時間（個/時） */
function periodAvgEfficiencyFromBucket(row: FormingProductivityBucket): number | null {
...
function sortByOperatorDefectRate(a: FormingProductivityOperatorRow, b: FormingProductivityOperatorRow): number {
...
function rankOverviewRowClass({ row }: { row: FormingProductivityProductOperatorRanking }): string {
...
function buildOperatorProductRows(
  ...
  const map = new Map<string, FormingProductivityProductRow & { sum_net_production_sec: number }>()
  for (const s of sessions) {
...
function buildReportFilters(): FormingProductivityReportFilters | null {
...
function buildReportContext(): FormingProductivityReportContext | null {
...
async function handleReportCommand(command: string | number | object) {
  ...
  try {
    ...
    if (cmd === 'print-full') {
      printFormingProductivityReport(ctx)
      return
    }

    const sectionMap: Record<string, FormingProductivityPrintSection> = {
      'print-daily': 'daily',
    ...
    if (section) {
      printFormingProductivitySection(section, ctx)
      return
    }
  } catch (err) {
...
async function captureDailyChartDataUrl(daily: FormingProductivityDailyRow[]): Promise<string | null> {
...
async function handleBatchDailyByOperatorPrint() {
  ...
  const items: FormingProductivityDailyBatchItem[] = []
  for (const [idx, op] of operators.entries()) {
    const res = await fetchFormingProductivityAnalysis({
      start_date: start,
      end_date: end,
  ...
  printFormingProductivityDailyBatch(filters, items)
}
...
async function handleBatchOperatorProductPrint() {
  ...
  const items: FormingProductivityOperatorProductBatchItem[] = []
  ...
  printFormingProductivityOperatorProductBatch(filters, items)
}
...
async function loadProductOptions() {
  ...
  try {
    ...
    productOptions.value = filterFormingSelectableProducts(list)
  } catch {
...
function defectLabel(defectCd: string): string {
  ...
  return defectLabelMap.value.get(cd) ?? resolveFormingMetricsLabel(cd, defectLabelMap.value)
}
...
async function loadLines() {
  ...
  try {
    lineOptions.value = await loadFormingLineOptions({ start_date: start, end_date: end })
