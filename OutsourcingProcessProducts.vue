    ...
    if (res?.success) {
      ...
      stats.value = {
        ...
        active:
          ...
          data.active_count ||
          tableData.value.filter((item: any) => item.is_active).length ||
          0,
        suppliers:
          data.total?.supplier_count ||
          data.supplier_count ||
          new Set(tableData.value.map((item: any) => item.supplier_cd)).size ||
          0,
      }
      ...
      stats.value = {
        total: data.total?.total_count || data.total_count || 0,
        active: data.total?.active_count || data.active_count || 0,
        suppliers: data.total?.supplier_count || data.supplier_count || 0,
      }
      ...
      // 如果API失败，从表格数据计算
      const uniqueSuppliers = new Set(tableData.value.map((item: any) => item.supplier_cd))
      stats.value = {
        total: tableData.value.length,
        active: tableData.value.filter((item: any) => item.is_active).length,
...
const fetchSuppliers = async () => {
  ...
  try {
    ...
    if (res?.success || res?.data?.success) {
      ...
      suppliersList.value = Array.isArray(data) ? data : data.data || []
    } else if (Array.isArray(res)) {
...
const handleAdd = () => {
  isEdit.value = false
  resetForm()
  // 如果当前在特定工程tab，自动设置工程类型
  if (activeProcessType.value !== 'all') {
    formData.process_type = activeProcessType.value as any
  }
  // 加载下拉框数据
  fetchSuppliers()
  fetchProducts()
...
.table-card :deep(.el-table--small) {
  .el-table__cell {
    padding: 4px 0;
  }
}
