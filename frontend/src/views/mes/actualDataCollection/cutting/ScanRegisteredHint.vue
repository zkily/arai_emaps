  ...
  >
    <template #reference>
      <span class="scan-registered-hint" role="button" tabindex="0" @click.stop>
        {{ t('mesCuttingActual.scanRegistered') }}
      </span>
    </template>
    <div class="plan-scan-tip">
      <p class="plan-scan-tip__title">{{ t('mesCuttingActual.scanCurrentCodeLabel') }}</p>
      <p class="plan-scan-tip__code">{{ scannedCode }}</p>
      <p v-if="(managementCode ?? '').trim()" class="plan-scan-tip__sub">
        {{ t('mesCuttingActual.managementCode') }}: {{ managementCode }}
      </p>
      <p class="plan-scan-tip__note">{{ t('mesCuttingActual.scanOverwriteHint') }}</p>
    </div>
  </el-popover>
</template>
