import type {
  ChamferingProductivityAnalysisData,
  ChamferingProductivityOperatorRow,
  ChamferingProductivityProductOperatorRanking,
  ChamferingProductivityProductRow,
  ChamferingProductivitySessionRow,
} from '@/api/chamferingProductionIndicator'

export interface ChamferingProductivityReportFilters {
  startDate: string
  endDate: string
  lineLabel: string
  productLabel: string
...
export type ChamferingProductivityKpiTone = 'indigo' | 'sky' | 'amber' | 'emerald' | 'violet'

export interface ChamferingProductivityKpiCardPrint {
  label: string
  value: string
  hint: string
  tone: ChamferingProductivityKpiTone
}

export interface ChamferingProductivityPrintCharts {
  daily: string | null
  operator: string | null
  product: string | null
}

export interface ChamferingProductivityPrintChartsExtended extends ChamferingProductivityPrintCharts {
  productRank?: string | null
}

export interface ChamferingProductivityProductRankPrintContext {
  selected: ChamferingProductivityProductOperatorRanking | null
  topOverview: ChamferingProductivityProductOperatorRanking[]
  stats: {
...
export interface ChamferingProductivityReportContext {
  filters: ChamferingProductivityReportFilters
  kpiCards: ChamferingProductivityKpiCardPrint[]
  charts: ChamferingProductivityPrintChartsExtended
  lineRows: Array<ChamferingProductivityOperatorRow & { avg_efficiency_per_hour?: number | null }>
  productRows: Array<ChamferingProductivityProductRow & { avg_efficiency_per_hour?: number | null }>
  lineSectionAvgEfficiency: number | null
  productSectionTotalQty: number
  productRank: ChamferingProductivityProductRankPrintContext
}

export type ChamferingProductivityPrintSection =
  | 'full'
  | 'daily'
  | 'operator'
  | 'product'
...
/** @deprecated use ChamferingProductivityReportContext */
export type ChamferingProductivityPrintPayload = ChamferingProductivityReportContext
...
function reportFileBase(filters: ChamferingProductivityReportFilters): string {
  return `溶接生産性分析_${filters.startDate}_${filters.endDate}`
}

function metaCsvLines(filters: ChamferingProductivityReportFilters): string[] {
  return [
    csvLine(['# 溶接工程 — 生産性分析']),
    csvLine(['# 集計期間', `${filters.startDate} ～ ${filters.endDate}`]),
    csvLine(['# ライン', filters.lineLabel]),
...
export function buildSummaryCsv(
  ...
  extras: {
    lineRows: Array<ChamferingProductivityOperatorRow & { avg_efficiency_per_hour?: number | null }>
    productRows: Array<ChamferingProductivityProductRow & { avg_efficiency_per_hour?: number | null }>
    productRankTop: ChamferingProductivityProductOperatorRanking[]
  ...
  lines.push(csvLine(['CD', '製品名', 'TOPライン', '能率(個/時)', '対象人数', '生産数']))
...
function kpiCardHtml(card: ChamferingProductivityKpiCardPrint): string {
...
function buildOperatorPrintTable(
  rows: Array<ChamferingProductivityOperatorRow & { avg_efficiency_per_hour?: number | null }>,
  maxRows = 0,
): string {
...
function buildProductPrintTable(
  rows: Array<ChamferingProductivityProductRow & { avg_efficiency_per_hour?: number | null }>,
): string {
  if (!rows.length) return '<p class="empty">データなし</p>'
  const body = rows
    .map(
...
function buildProductRankOperatorTable(rows: ChamferingProductivityOperatorRow[]): string {
  if (!rows.length) return '<p class="empty">データなし</p>'
  const body = rows
    .map(
      (row) => `<tr>
...
function buildProductRankOverviewTable(rows: ChamferingProductivityProductOperatorRanking[]): string {
  if (!rows.length) return '<p class="empty">データなし</p>'
  const body = rows
    .map(
      (row) => `<tr>
  ...
  return `<table class="data data--rank-overview">
    <thead>${tableHead(['CD', '製品名', 'TOPライン', '能率', '対象人数', '生産'])}</thead>
    <tbody>${body}</tbody>
  </table>`
}

function buildProductRankHeroHtml(
  ranking: ChamferingProductivityProductOperatorRanking,
  stats: ChamferingProductivityProductRankPrintContext['stats'],
): string {
  return `<div class="rank-hero">
    ...
    <div class="rank-hero__stats">
      ...
      <div class="rank-hero__stat"><span class="rank-hero__stat-label">ライン</span><span class="rank-hero__stat-val">${escHtml(stats?.operatorCount ?? ranking.ranked_operator_count ?? 0)}<small>名</small></span></div>
...
function buildMetaLineHtml(
  ...
  return `<div class="meta-line">
      ...
      <span><span class="meta-line__label">出力日時</span> ${escHtml(printedAt)}</span>
      <span class="meta-line__sep">|</span>
      <span><span class="meta-line__label">ライン</span> ${escHtml(filters.lineLabel)}</span>
      <span class="meta-line__sep">|</span>
...
function buildFullPrintBody(ctx: ChamferingProductivityReportContext): string {
  const {
    charts,
    lineRows,
    productRows,
    lineSectionAvgEfficiency,
    productSectionTotalQty,
  } = ctx

  const operatorBadges = [
    panelBadge(`${lineRows.length} 名`, 'soft'),
    lineSectionAvgEfficiency != null
      ? panelBadge(`平均能率 ${fmtEfficiency(lineSectionAvgEfficiency)} 個/時`, 'inspector')
      : '',
  ].join('')
  ...
  ${panelSection('ライン別', { theme: 'operator', badges: operatorBadges, chartSrc: charts.operator, chartAlt: 'ライン別', tableHtml: buildOperatorPrintTable(lineRows, PRINT_OPERATOR_TABLE_MAX) })}
...
export function buildChamferingProductivityPrintHtml(ctx: ChamferingProductivityReportContext): string {
...
export function buildChamferingProductivitySectionPrintHtml(
  section: ChamferingProductivityPrintSection,
  ctx: ChamferingProductivityReportContext,
): string {
  const {
    charts,
    lineRows,
  ...
  if (section === 'full') {
    return buildChamferingProductivityPrintHtml(ctx)
  }
  ...
  if (section === 'operator') {
    ...
    return buildPrintDocumentShell(ctx.filters, {
      sectionTitle: 'ライン別',
      mode: 'section',
      body: panelSection('ライン別', {
        theme: 'operator',
        badges,
        chartSrc: charts.operator,
        chartAlt: 'ライン別',
        tableHtml: buildOperatorPrintTable(lineRows),
      }),
    })
  }
  ...
  if (section === 'product-rank') {
    ...
    const selectedBody = selected
      ? `${buildProductRankHeroHtml(selected, productRank.stats)}
        ${panelSection('ライン別能率', { theme: 'rank', badges: panelBadge(`${selected.operators.length} 名`, 'rank'), chartSrc: charts.productRank ?? null, chartAlt: 'ライン別能率', tableHtml: buildProductRankOperatorTable(selected.operators) })}`
...
export interface ChamferingProductivityDailyBatchItem {
...
export function buildChamferingProductivityDailyBatchPrintHtml(
  filters: ChamferingProductivityReportFilters,
  items: ChamferingProductivityDailyBatchItem[],
): string {
  const printedAt = printedAtJa()
  const body = items
    .map((item, idx) => {
...
<head>
  <meta charset="UTF-8" />
  <title>溶接工程 — 生産性分析 — 日別推移（ライン別）</title>
...
export function printChamferingProductivityDailyBatch(
  filters: ChamferingProductivityReportFilters,
  items: ChamferingProductivityDailyBatchItem[],
) {
  openPrintDocument(buildChamferingProductivityDailyBatchPrintHtml(filters, items))
}

export interface ChamferingProductivityOperatorProductBatchItem {
  lineLabel: string
  productCount: number
  sessionCount: number
  sumActualQty: number
...
export function buildChamferingProductivityOperatorProductBatchPrintHtml(
  filters: ChamferingProductivityReportFilters,
  items: ChamferingProductivityOperatorProductBatchItem[],
): string {
  const printedAt = printedAtJa()
  const body = items
    .map((item, idx) => {
...
<head>
  <meta charset="UTF-8" />
  <title>溶接工程 — 生産性分析 — ライン別製品別</title>
...
export function printChamferingProductivityOperatorProductBatch(
  filters: ChamferingProductivityReportFilters,
  items: ChamferingProductivityOperatorProductBatchItem[],
) {
  openPrintDocument(buildChamferingProductivityOperatorProductBatchPrintHtml(filters, items))
}

export function printChamferingProductivityReport(payload: ChamferingProductivityPrintPayload) {
  openPrintDocument(buildChamferingProductivityPrintHtml(payload))
}

export function printChamferingProductivitySection(
  section: ChamferingProductivityPrintSection,
  ctx: ChamferingProductivityReportContext,
) {
  openPrintDocument(buildChamferingProductivitySectionPrintHtml(section, ctx))
}
