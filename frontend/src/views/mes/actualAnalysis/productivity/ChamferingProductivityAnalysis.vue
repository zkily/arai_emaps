    ...
    <header class="ipa-hero ipa-fade-in">
      <div class="ipa-hero__main">
        ...
        <div class="ipa-hero__text">
          ...
          <h1 class="ipa-hero__title">面取工程 — 生産性分析</h1>
    ...
    <div class="ipa-toolbar ipa-panel ipa-fade-in ipa-fade-in--d1">
      <div class="ipa-toolbar__fields">
        ...
        <div class="ipa-field ipa-field--inspector">
          <span class="ipa-field__pill"><el-icon><User /></el-icon>ライン</span>
          <el-select
            ...
            <section class="ipa-panel ipa-panel--inspector ipa-fade-in ipa-fade-in--d4">
              <div class="ipa-panel__head">
                <div class="ipa-panel__title-wrap">
                  ...
                  <span class="ipa-panel__title">ライン別</span>
                </div>
              ...
              >
                ...
                <el-table-column prop="operator_name" label="ライン" min-width="96" show-overflow-tooltip />
          ...
          <section v-if="productRankList.length" class="ipa-panel ipa-panel--rank ipa-fade-in ipa-fade-in--d5b">
            <div class="ipa-panel__head ipa-rank-panel__head">
              <div class="ipa-panel__title-wrap">
                ...
                <div>
                  <span class="ipa-panel__title">製品別 · ライン能率ランキング</span>
                  <div class="ipa-rank-panel__desc">製品ごとのライン能率を順位付けして比較</div>
                </div>
              </div>
              <div class="ipa-panel__badges">
              ...
              <div v-if="selectedProductRanking" :key="rankViewProductCd" class="ipa-rank-body">
                <div class="ipa-rank-hero">
                  ...
                  <div v-if="selectedProductRankStats" class="ipa-rank-hero__stats">
                    ...
                    <div class="ipa-rank-hero__stat">
                      <span class="ipa-rank-hero__stat-label">ライン</span>
                ...
                <div class="ipa-rank-chart-block">
                  <div class="ipa-rank-chart-block__head">
                    <span class="ipa-rank-chart-block__title">ライン別能率</span>
                ...
                >
                  ...
                  <el-table-column prop="operator_name" label="ライン" min-width="120" show-overflow-tooltip />
            ...
            <div class="ipa-rank-overview">
              ...
              >
                ...
                <el-table-column label="TOPライン" min-width="110" show-overflow-tooltip>
                  <template #default="{ row }">
...
import {
  fetchChamferingProductivityAnalysis,
  type ChamferingProductivityAnalysisData,
  type ChamferingProductivityBucket,
  type ChamferingProductivityDailyRow,
  type ChamferingProductivityOperatorRow,
  type ChamferingProductivityProductOperatorRanking,
  type ChamferingProductivityProductRow,
  type ChamferingProductivitySessionRow,
} from '@/api/chamferingProductionIndicator'
import type { ChamferingProductionIndicatorLineOption as ChamferingLineOption } from '@/api/chamferingProductionIndicator'
import { getProductList } from '@/api/master/productMaster'
import type { Product } from '@/types/master'
import { getJSTToday, parseDateAsJST } from '@/utils/dateFormat'
import { lineLabel, loadChamferingLineOptions, resolveChamferingMetricsLabel } from '@/views/mes/shared/chamferingLineFilter'
...
import {
  printChamferingProductivityDailyBatch,
  printChamferingProductivityOperatorProductBatch,
  printChamferingProductivityReport,
  printChamferingProductivitySection,
  type ChamferingProductivityDailyBatchItem,
  type ChamferingProductivityOperatorProductBatchItem,
  type ChamferingProductivityPrintSection,
  type ChamferingProductivityReportContext,
  type ChamferingProductivityReportFilters,
} from './chamferingProductivityReport'

defineOptions({ name: 'MesChamferingProductivityAnalysis' })
...
const analysisData = ref<ChamferingProductivityAnalysisData | null>(null)
const lineOptions = ref<ChamferingLineOption[]>([])
const filterLineName = ref<string>('')
...
const emptySummary = (): ChamferingProductivityBucket => ({
  session_count: 0,
...
const productRankList = computed((): ChamferingProductivityProductOperatorRanking[] => {
...
function buildProductOperatorRankingFromSessions(
  sessions: ChamferingProductivityAnalysisData['sessions'],
  ...
  const productMap = new Map<
    string,
    {
      ...
      operators: Map<string, ChamferingProductivityOperatorRow>
    }
  >()
  ...
  const result: ChamferingProductivityProductOperatorRanking[] = []
  for (const prod of productMap.values()) {
    const operators: ChamferingProductivityOperatorRow[] = []
...
const podiumOperators = computed(() => {
  ...
  return order
    ...
    .filter((r): r is ChamferingProductivityOperatorRow => r != null)
})
...
function productRankOptionLabel(p: ChamferingProductivityProductOperatorRanking): string {
...
function sortByEfficiency(a: ChamferingProductivityOperatorRow, b: ChamferingProductivityOperatorRow): number {
...
type OperatorDisplayRow = ChamferingProductivityOperatorRow & {
  avg_efficiency_per_hour: number | null
}

type ProductDisplayRow = ChamferingProductivityProductRow & {
  avg_efficiency_per_hour: number | null
}

/** 期間内：生産合計 ÷ 正味稼働時間（個/時） */
function periodAvgEfficiencyFromBucket(row: ChamferingProductivityBucket): number | null {
...
function sortByOperatorDefectRate(a: ChamferingProductivityOperatorRow, b: ChamferingProductivityOperatorRow): number {
...
function rankOverviewRowClass({ row }: { row: ChamferingProductivityProductOperatorRanking }): string {
...
function buildOperatorProductRows(
  ...
  const map = new Map<string, ChamferingProductivityProductRow & { sum_net_production_sec: number }>()
  for (const s of sessions) {
...
function buildReportFilters(): ChamferingProductivityReportFilters | null {
...
function buildReportContext(): ChamferingProductivityReportContext | null {
...
async function handleReportCommand(command: string | number | object) {
  ...
  try {
    ...
    if (cmd === 'print-full') {
      printChamferingProductivityReport(ctx)
      return
    }

    const sectionMap: Record<string, ChamferingProductivityPrintSection> = {
      'print-daily': 'daily',
    ...
    if (section) {
      printChamferingProductivitySection(section, ctx)
      return
    }
  } catch (err) {
...
async function captureDailyChartDataUrl(daily: ChamferingProductivityDailyRow[]): Promise<string | null> {
...
async function handleBatchDailyByOperatorPrint() {
  ...
  const operators = lineOptions.value
  if (!operators.length) {
    ElMessage.warning('ラインが登録されていません')
    return
  }

  const items: ChamferingProductivityDailyBatchItem[] = []
  for (const [idx, op] of operators.entries()) {
    const res = await fetchChamferingProductivityAnalysis({
      start_date: start,
      end_date: end,
  ...
  printChamferingProductivityDailyBatch(filters, items)
}
...
async function handleBatchOperatorProductPrint() {
  ...
  const items: ChamferingProductivityOperatorProductBatchItem[] = []
  ...
  if (!items.length) {
    ElMessage.warning('印刷できるライン別製品データがありません')
    return
  }

  items.sort((a, b) => (b.avgEfficiencyPerHour ?? -1) - (a.avgEfficiencyPerHour ?? -1))
  printChamferingProductivityOperatorProductBatch(filters, items)
}
...
async function loadProductOptions() {
  ...
  try {
    ...
    productOptions.value = filterChamferingSelectableProducts(list)
  } catch {
...
function defectLabel(defectCd: string): string {
  ...
  return defectLabelMap.value.get(cd) ?? resolveChamferingMetricsLabel(cd, defectLabelMap.value)
}
...
async function loadLines() {
  ...
  try {
    lineOptions.value = await loadChamferingLineOptions({ start_date: start, end_date: end })
...
onMounted(async () => {
  ...
  await Promise.all([loadLines(), loadDefectLabels(), loadProductOptions()])
