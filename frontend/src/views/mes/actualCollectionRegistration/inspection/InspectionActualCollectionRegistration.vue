...
import { useInspectionManualRegistration } from './useInspectionManualRegistration'
...
const inspectorSelected = computed(() => {
  const id = form.value.inspectorUserId
...
function onStopMinEnter(e: Event): void {
  if (!(e instanceof KeyboardEvent) || e.key !== 'Enter') return
  e.preventDefault()
  focusDefectById(kakouKizuDefectId.value)
}
    ...
    <header class="iar-hero iar-rise">
      <div class="iar-hero__main">
        ...
        <div>
          ...
          <h1 class="iar-hero__title">検査実績収集登録</h1>
        </div>
      </div>
      ...
      <div class="iar-form">
        <div class="iar-form__row iar-form__row--triple">
          ...
          <div class="iar-field iar-field--c2">
            <label class="iar-field__label"><span class="iar-step iar-step--2">②</span>検査員</label>
            <el-select
        ...
        <p v-if="!isEdit && !inspectorSelected" class="iar-form__lock-hint iar-form__lock-hint--pulse">
          <el-icon><Warning /></el-icon>
          ② 検査員を選択すると、③ 製品名を入力できます
        </p>
        ...
        <div class="iar-defects iar-field--c6">
          <label class="iar-field__label">
            <span class="iar-step iar-step--6">⑥</span>不良（項目別）
    ...
    <section class="iar-panel iar-panel--table iar-rise iar-rise--d2">
      ...
      <div class="iar-table-wrap">
        ...
        >
          ...
          <el-table-column label="検査員" width="96" align="left" header-align="left" show-overflow-tooltip>
            <template #default="{ row }">{{ inspectorLabel(row.mes_inspector_user_id) }}</template>
          </el-table-column>
