"""
生産管理指標(YYYY年度-溶接).csv/.xlsx「入力表」→ welding_management 取込（CLI）

用法（仓库根目录，需 backend/.env 中 DB_* 已配置）:

    py scripts/import_welding_management_csv.py --dry-run
    py scripts/import_welding_management_csv.py --csv "frontend/生産管理指標(2026年度-溶接).csv"
    py scripts/import_welding_management_csv.py --replace
"""
...
from app.services.welding_management_import import (  # noqa: E402
    REMARKS_PREFIX_CSV,
...
DEFAULT_CSV = ROOT / "frontend" / "生産管理指標(2026年度-溶接).csv"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="CSV/Excel → welding_management")
    ...
    try:
        ...
        rows, errors = parse_rows_from_source(
            str(source),
            user_rows=user_rows,
            skip_unmapped_operator=args.skip_unmapped,
        ...
        if result.unmapped_operators:
            print("未匹配作業者:")
            for name, cnt in sorted(result.unmapped_operators.items(), key=lambda x: -x[1]):
                print(f"  - {name}: {cnt} 行")
        if result.errors:
