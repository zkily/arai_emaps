...
import {
  fetchChamferingManagementList,
  fetchChamferingMesMachines,
  patchChamferingManagement,
  reorderChamferingManagement,
  splitChamferingManagementToNextDay,
  type ChamferingManagementListRow,
  type ChamferingMesMachine,
  type PatchChamferingManagementBody,
} from '@/api/chamferingManagement'
...
import {
  ...
  loadChamferingActualPersist,
  makePersistScopeKey,
  reconcileInProgressTimer,
  saveChamferingActualPersist,
  serializePlanSessions,
} from './chamferingActualPersist'
import {
...
const machines = ref<ChamferingMesMachine[]>([])
const managementRows = ref<ChamferingManagementListRow[]>([])
...
type ChamferingMgmtRow = ChamferingManagementListRow & { id: number }
...
async function patchWithOfflineSync(
  ...
  if (!navigator.onLine) {
    ...
    if (!options?.silentQueue) {
      ElMessage.warning(t('mesChamferingActual.offlineQueued'))
    }
    return false
  }
  try {
    const res = await patchChamferingManagement(planId, body)
    if (res && res.success === false) {
      if (!options?.silentQueue) {
        ElMessage.warning(res.message || t('mesChamferingActual.saveFailed'))
      }
      return false
    }
    return true
...
async function tryFlushOfflineQueue(options?: { reloadAfter?: boolean }): Promise<void> {
  ...
  try {
    const { ok, fail } = await flushOfflinePatchQueue(patchChamferingManagement)
    refreshOfflineQueueCount()
    if (ok > 0) {
      ElMessage.success(t('mesChamferingActual.offlineSyncOk', { n: ok }))
      if (options?.reloadAfter && selectedMachineId.value != null) {
        await loadPlans()
      }
    }
    if (fail > 0) {
      ElMessage.warning(t('mesChamferingActual.offlineSyncPartial'))
    }
  } finally {
...
const offlineAlertText = computed(() => {
  if (isBrowserOffline.value) return t('mesChamferingActual.offlineBanner')
  return t('mesChamferingActual.offlinePendingSync', { n: offlineQueueCount.value })
})
...
function chamferingMachineHasGaichu(m: ChamferingMesMachine): boolean {
  const name = m.machine_name ?? ''
  const cd = m.machine_cd ?? ''
  return name.includes('外注') || cd.includes('外注')
}

/** 面取設備プルダウン用（外注除外・API 生データは machines をそのまま保持） */
const chamferingMachinesForSelect = computed(() => machines.value.filter((m) => !chamferingMachineHasGaichu(m)))

watch(chamferingMachinesForSelect, (list) => {
  const id = selectedMachineId.value
...
function formatGroupDayLabel(dayKey: string): string {
  if (dayKey === '—') return t('mesChamferingActual.noProductionDay')
...
function formatMgmtCodeShort(code: string | null | undefined): string {
  ...
  return t('mesChamferingActual.mgmtCodeShort', { code: tail })
}

/** 生産日ごとのグループ表示（loadPlans は production_day で絞り込み済み） */
const visibleRowsByDay = computed(() => {
  let rows = managementRows.value.filter((r): r is ChamferingMgmtRow => r.id != null)
  ...
  const groups = new Map<string, ChamferingMgmtRow[]>()
  for (const r of rows) {
...
function applyLocalProductionSequence(orderedRows: ChamferingMgmtRow[]): void {
...
async function onPlanCardSortEnd(dayKey: string, evt: SortableEvent): Promise<void> {
  ...
  if (!machine) return
  if (!navigator.onLine) {
    ElMessage.warning(t('mesChamferingActual.needOnlineForEdit'))
    await loadPlans()
    return
  }
  ...
  destroyPlanSortables()
  try {
    const res = await reorderChamferingManagement({ production_day: (productionDay.value ?? '').trim().slice(0, 10), chamfering_machine: machine, ordered_ids: orderedIds })
    if (res && res.success === false) {
      throw new Error(res.message || 'reorder failed')
    }
    ElMessage.success(t('mesChamferingActual.reorderSaved'))
  } catch (e: unknown) {
    console.error(e)
    ElMessage.error(t('mesChamferingActual.reorderFailed'))
    await loadPlans()
  } finally {
...
function isChamferingRowConfirmed(row: ChamferingMgmtRow): boolean {
  return Number(row.production_completed_check ?? 0) === 1
}

function isChamferingRowMesEnded(row: ChamferingMgmtRow): boolean {
  const endedAt = row.mes_production_ended_at
  return endedAt != null && String(endedAt).trim() !== ''
}

function isChamferingRowSessionEnded(row: ChamferingMgmtRow): boolean {
  if (row.id == null) return false
  return ensureSession(row.id).wallEnd != null
}

/** 終了済（画面 or DB）または実績確定済 — 表示・計画数ラベル用 */
function isChamferingRowConfirmedForDisplay(row: ChamferingMgmtRow): boolean {
  return isChamferingRowConfirmed(row) || isChamferingRowSessionEnded(row) || isChamferingRowMesEnded(row)
}

function statusTagType(row: ChamferingMgmtRow): TagType {
  if (isChamferingRowConfirmedForDisplay(row)) return 'success'
  return 'warning'
}

function statusText(row: ChamferingMgmtRow): string {
  if (isChamferingRowConfirmedForDisplay(row)) return t('mesChamferingActual.cmConfirmed')
  return t('mesChamferingActual.cmPending')
}

function canScanBarcodeForRow(row: ChamferingMgmtRow): boolean {
  return !isChamferingRowConfirmedForDisplay(row) && row.id != null
}

function rowHasScanRegistration(row: ChamferingMgmtRow): boolean {
...
function openBarcodeScanDialog(row: ChamferingMgmtRow): void {
  if (!canScanBarcodeForRow(row)) return
  barcodeScanTargetRow.value = row
  barcodeScanDialogVisible.value = true
}

function barcodeScanProductLabel(row: ChamferingMgmtRow | null): string {
  if (!row) return ''
...
async function onBarcodeScanned(code: string): Promise<void> {
  ...
  try {
    ...
    if (ok) {
      ...
      if (replaced) {
        ElMessage.success(
          resolved ? t('mesChamferingActual.scanReplacedRegistered') : t('mesChamferingActual.scanReplaced'),
        )
      } else {
        ElMessage.success(
          resolved ? t('mesChamferingActual.scanRegistered') : t('mesChamferingActual.scanSaved'),
        )
      }
    }
  } catch (e: unknown) {
    console.error(e)
    ElMessage.error(t('mesChamferingActual.saveFailed'))
  } finally {
...
const confirmedEditRow = ref<ChamferingMgmtRow | null>(null)
...
function buildClearMesFieldsPatchBody(): PatchChamferingManagementBody {
  return {
...
const barcodeScanTargetRow = ref<ChamferingMgmtRow | null>(null)
...
async function submitConfirmedEdit(): Promise<void> {
  ...
  if (draft.actualQty != null && (!Number.isFinite(draft.actualQty) || draft.actualQty < 0)) {
    ElMessage.warning(t('mesChamferingActual.qtyInvalid'))
    return
  }
  ...
  if (ws == null || !Number.isFinite(ws) || we == null || !Number.isFinite(we)) {
    ElMessage.warning(t('mesChamferingActual.editTimeRequired'))
    return
  }
  if (we < ws) {
    ElMessage.warning(t('mesChamferingActual.editTimeOrder'))
    return
  }
  ...
  if (!Number.isFinite(pauseSec) || pauseSec > wallSec) {
    ElMessage.warning(t('mesChamferingActual.editPauseTooLong'))
    return
  }
  ...
  const body: PatchChamferingManagementBody = {
  ...
  try {
    ...
    if (!ok) return
    ElMessage.success(t('mesChamferingActual.confirmedEditSaved'))
    clearConfirmedEditState()
...
async function clearConfirmedMesAndSave(): Promise<void> {
  ...
  try {
    await ElMessageBox.confirm(
      t('mesChamferingActual.clearMesConfirmMessage'),
      t('mesChamferingActual.clearMesConfirmTitle'),
      {
        type: 'warning',
        confirmButtonText: t('mesChamferingActual.btnClearMesActual'),
  ...
  try {
    ...
    if (!ok) return
    ElMessage.success(t('mesChamferingActual.clearMesSaved'))
    clearConfirmedEditState()
    flushPersistToStorage()
    await loadPlans()
  } catch (e: unknown) {
...
function rowQtyChipLabel(row: ChamferingMgmtRow): string {
  return isChamferingRowConfirmedForDisplay(row) ? t('mesChamferingActual.actualQty') : t('mesChamferingActual.plannedQty')
}

function rowQtyChipValue(row: ChamferingMgmtRow): string | number {
...
function timerPhaseLabel(sess: PlanSession): string {
  ...
  if (ph === 'idle') return t('mesChamferingActual.timerIdle')
  if (ph === 'running') return t('mesChamferingActual.timerRunning')
  if (ph === 'paused') return t('mesChamferingActual.timerPaused')
  return t('mesChamferingActual.timerEnded')
}
...
function isRowMesProductionActive(row: ChamferingMgmtRow): boolean {
...
/** 同一設備（現在の一覧）で他に生産中の行 */
function findOtherActiveProductionRow(excludePlanId: number): ChamferingMgmtRow | null {
  for (const row of managementRows.value) {
    if (row.id == null || row.id === excludePlanId) continue
    const mgmt = row as ChamferingMgmtRow
    if (isRowMesProductionActive(mgmt)) return mgmt
  }
  return null
}

function rowProductionShortLabel(row: ChamferingMgmtRow): string {
...
async function onStart(planId: number) {
  ...
  if (s.wallStart == null) {
    ...
    if (other) {
      ElMessage.warning(
        t('mesChamferingActual.singleMachineProductionOnly', {
    ...
    try {
      ...
      if (status === 409 && typeof detail === 'string' && detail.trim()) {
        ...
        ElMessage.error(
          typeof detail === 'string' && detail.trim() ? detail.trim() : t('mesChamferingActual.saveFailed'),
        )
      }
...
function sortedRowIdsBySequence(rows: ChamferingManagementListRow[]): number[] {
  return [...rows]
    .filter((r): r is ChamferingManagementListRow & { id: number } => r.id != null)
...
async function reorderMachineRowsForDay(machineName: string, productionDay: string): Promise<void> {
  ...
  const res = await fetchChamferingManagementList({
  ...
  const reorderRes = await reorderChamferingManagement({ production_day: productionDay, chamfering_machine: cm, ordered_ids: ids })
...
async function reorderAfterMachineMove(params: {
  ...
  const targetRes = await fetchChamferingManagementList({
    production_day: day,
    chamfering_machine: target,
    limit: 2000,
  })
  ...
  const reorderRes = await reorderChamferingManagement({
    production_day: day,
    chamfering_machine: target,
...
function splitBaselineQty(row: ChamferingMgmtRow): number {
  const n = Number(row.actual_production_quantity ?? 0)
  return Number.isFinite(n) && n >= 0 ? Math.floor(n) : 0
}

/** 不良数 = 生産終了時の確認本数 − 確定前の actual_production_quantity */
function computedDefectQty(row: ChamferingMgmtRow, confirmedQty: number): number {
...
function plannedHintQty(row: ChamferingMgmtRow): number {
  const n = Number(row.actual_production_quantity ?? 0)
  return Number.isFinite(n) && n >= 0 ? Math.floor(n) : 0
}
...
const endDialogRow = ref<ChamferingMgmtRow | null>(null)
...
const changeMachineRow = ref<ChamferingMgmtRow | null>(null)
...
const changeMachineCurrentName = computed((): string => {
  ...
  if (!row) return '—'
  return (row.chamfering_machine ?? '').trim() || '—'
})

function openChangeMachineDialog(row: ChamferingMgmtRow): void {
  if (!canChangeMachine(row) || row.id == null) return
  changeMachineRow.value = row
  const currentName = (row.chamfering_machine ?? '').trim()
  const matched = chamferingMachinesForSelect.value.find(
...
async function submitChangeMachine(): Promise<void> {
  ...
  if (!canChangeMachine(row)) {
    ElMessage.warning(t('mesChamferingActual.changeMachineForbidden'))
    return
  }
  if (!navigator.onLine) {
    ElMessage.warning(t('mesChamferingActual.needOnlineForEdit'))
  ...
  if (!targetName) {
    ElMessage.warning(t('mesChamferingActual.emptyMachineName'))
    return
  }
  const sourceName = (row.chamfering_machine ?? '').trim() || selectedMachineName.value || ''
  if (!sourceName) {
    ElMessage.warning(t('mesChamferingActual.emptyMachineName'))
    return
  }
  if (targetName === sourceName) {
    ElMessage.warning(t('mesChamferingActual.changeMachineSame'))
    return
  }
  const dayStr = normalizeProductionDayKey(row.production_day ?? productionDay.value)
  if (dayStr === '—' || !/^\d{4}-\d{2}-\d{2}$/.test(dayStr)) {
    ElMessage.warning(t('mesChamferingActual.invalidProductionDay'))
    return
  }

  changeMachineSubmitting.value = true
  try {
    const ok = await patchWithOfflineSync(row.id, { chamfering_machine: targetName })
    if (!ok) return
    ...
    ElMessage.success(t('mesChamferingActual.changeMachineSaved', { machine: targetName }))
...
function openProductionEndDialog(row: ChamferingMgmtRow) {
...
async function deferSingleRowToNextDay(
  ...
  if (planId != null) {
    markLocalMesEcho(planId)
    const mesPatch: PatchChamferingManagementBody = {
  ...
  const res = await splitChamferingManagementToNextDay(row.id, {
  ...
  await patchChamferingManagement(row.id, { production_completed_check: true })
}

/** 後続計画は split せず生産日のみ変更（数量・行はそのまま） */
async function moveSubsequentRowsProductionDay(rows: ChamferingMgmtRow[], targetDay: string): Promise<void> {
  for (const subRow of rows) {
    if (subRow.id == null) continue
    const res = await patchChamferingManagement(subRow.id, { production_day: targetDay })
    if (res && res.success === false) {
...
async function reorderTargetDayAfterDefer(
  ...
  const all = (res.data ?? []).filter((r): r is ChamferingManagementListRow & { id: number } => r.id != null)
  if (all.length === 0) return

  const subsequentSet = new Set(subsequentIds)
  const currentId = currentRow.id
  const mgmt = (currentRow.management_code ?? '').trim()
  const sameProduct = (r: ChamferingManagementListRow): boolean => {
  ...
  const reorderRes = await reorderChamferingManagement({ production_day: targetDay, chamfering_machine: cm, ordered_ids: orderedIds })
  if (reorderRes && reorderRes.success === false) {
    throw new Error(reorderRes.message || 'reorder failed')
  }
}
...
async function reorderAfterProductionDefer(
  currentRow: ChamferingMgmtRow,
  targetDay: string,
  subsequentRows: ChamferingMgmtRow[],
): Promise<void> {
  const machine = (currentRow.chamfering_machine ?? selectedMachineName.value ?? '').trim()
  if (!machine) return
...
async function submitProductionEndFullBaseline() {
  ...
  if (!row || planId == null) return
  if (!navigator.onLine) {
    ElMessage.warning(t('mesChamferingActual.needOnlineForEnd'))
    return
  }
  const baseline = splitBaselineQty(row)
  if (baseline <= 0) {
    ElMessage.warning(t('mesChamferingActual.splitBaselineZero'))
    return
  }
  ...
  try {
    ...
    ElMessage.success(t('mesChamferingActual.completeSaved'))
...
async function submitProductionEndDefer() {
  ...
  if (qty >= baseline) {
    ElMessage.warning(t('mesChamferingActual.deferQtyMustBeLess'))
    return
  }
  const nextDay = (endDialogDeferDay.value ?? '').trim().slice(0, 10)
  if (!/^\d{4}-\d{2}-\d{2}$/.test(nextDay)) {
    ElMessage.warning(t('mesChamferingActual.deferNextDayRequired'))
    return
  }
  ...
  try {
    await ElMessageBox.confirm(
      subsequentCount > 0
        ? t('mesChamferingActual.deferConfirmWithFollowing', {
            date: nextDay,
            n: subsequentCount,
            remainder: endDialogDeferRemainder.value,
          })
        : t('mesChamferingActual.deferConfirmMessage', {
            date: nextDay,
            remainder: endDialogDeferRemainder.value,
          }),
      t('mesChamferingActual.deferConfirmTitle'),
      {
        type: 'warning',
        confirmButtonText: t('mesChamferingActual.btnDeferConfirm'),
        cancelButtonText: t('common.cancel'),
      },
    )
  } catch {
  ...
  try {
    ...
    ElMessage.success(
      subsequentCount > 0
        ? t('mesChamferingActual.deferSavedWithFollowing', { n: subsequentCount + 1, date: nextDay })
        : t('mesChamferingActual.deferSavedWithDate', { date: nextDay }),
    )
    closeProductionEndDialog()
    await loadPlans()
  } catch (e: unknown) {
    ...
    ElMessage.error(msg ? String(msg) : t('mesChamferingActual.saveFailed'))
    await loadPlans()
  } finally {
    endDialogSubmitting.value = false
  }
...
async function loadMachines() {
  ...
  try {
    machines.value = await fetchChamferingMesMachines()
  } catch (e: unknown) {
    console.error(e)
    ElMessage.error(t('mesChamferingActual.loadMachinesFailed'))
  } finally {
...
async function loadOperators() {
  ...
  try {
    ...
    console.error(e)
    ElMessage.error(t('mesChamferingActual.loadUsersFailed'))
  } finally {
...
async function syncMesStateFromServer(): Promise<void> {
  ...
  const chamferingMachine = selectedMachineName.value
  if (!chamferingMachine) return
  const dayStr = (productionDay.value ?? '').trim()
  ...
  try {
    const res = await fetchChamferingManagementList({
      ...
      chamfering_machine: chamferingMachine,
      limit: 2000,
    })
...
async function loadPlans() {
  if (selectedMachineId.value == null) {
    ElMessage.warning(t('mesChamferingActual.emptySelectMachine'))
    return
  }
  const chamferingMachine = selectedMachineName.value
  if (!chamferingMachine) {
    ElMessage.warning(t('mesChamferingActual.emptyMachineName'))
    return
  }
  ...
  try {
    ...
    if (!res.success) {
      ElMessage.error(res.message || t('mesChamferingActual.loadPlansFailed'))
      if (managementRows.value.length === 0) {
        managementRows.value = []
        clearSessions()
      } else {
        reapplyLocalSessionsForCurrentScope()
        ElMessage.warning(t('mesChamferingActual.offlineListCached'))
      }
      return
    }
    const rows = res.data ?? []
    ...
    if (restored) {
      ElMessage.info(t('mesChamferingActual.stateRestored'))
    }
    flushPersistToStorage()
    ...
    const detail =
      ...
      (e as { response?: { data?: { detail?: string } } }).response?.data?.detail
    ElMessage.error(
      typeof detail === 'string' && detail.trim() ? detail : t('mesChamferingActual.loadPlansFailed'),
    )
    if (managementRows.value.length > 0) {
      reapplyLocalSessionsForCurrentScope()
      ElMessage.warning(t('mesChamferingActual.offlineListCached'))
...
function restorePageFiltersFromPersist(): void {
  const blob = loadChamferingActualPersist()
  if (!blob) return
...
onMounted(async () => {
  ...
  if (
    machineId != null &&
    chamferingMachinesForSelect.value.some((m) => m.id === machineId)
  ) {
...
<template>
  <div class="cutting-actual-page">
      ...
      <div class="page-head-row">
        <div class="page-head-main">
          <h1 class="page-title">
            ...
            <span class="page-title__text">{{ t('mesChamferingActual.title') }}</span>
          </h1>
        </div>
        <div
          class="page-head-locale"
          role="radiogroup"
          :aria-label="t('mesChamferingActual.localeInline')"
        >
          <el-tooltip
    ...
    <el-card shadow="never" class="toolbar-card" role="region" :aria-label="t('mesChamferingActual.title')">
      <div class="toolbar-layout">
        <div class="toolbar-field-row toolbar-field-row--day">
          <span class="toolbar-field-row__icon" aria-hidden="true">
            <el-icon><Calendar /></el-icon>
          </span>
          <span class="toolbar-field-row__label">{{ t('mesChamferingActual.productionDay') }}</span>
          <div class="toolbar-day-wrap">
            ...
            <div class="toolbar-day-shortcuts">
              <el-tooltip :content="t('mesChamferingActual.dayPrev')" placement="top">
                <el-button
                  type="default"
                  size="small"
                  circle
                  :icon="ArrowLeft"
                  :aria-label="t('mesChamferingActual.dayPrev')"
              ...
              >
                {{ t('mesChamferingActual.dayToday') }}
              </el-button>
              <el-tooltip :content="t('mesChamferingActual.dayNext')" placement="top">
                <el-button
                  type="default"
                  size="small"
                  circle
                  :icon="ArrowRight"
                  :aria-label="t('mesChamferingActual.dayNext')"
                  @click="shiftProductionDay(1)"
                />
              </el-tooltip>
            </div>
        ...
        <div class="toolbar-field-row toolbar-field-row--machine">
          ...
          <span class="toolbar-field-row__label">{{ t('mesChamferingActual.machine') }}</span>
          <div class="toolbar-machine-wrap">
            ...
            clearable
            teleported
            :placeholder="t('mesChamferingActual.machinePlaceholder')"
          ...
          >
            <el-option
              v-for="m in chamferingMachinesForSelect"
              :key="m.id"
        ...
        >
          <span class="toolbar-field-row__label toolbar-filter-switch__text">{{
            t('mesChamferingActual.hideCompleted')
          }}</span>
          <el-switch
            v-model="hideCompleted"
            class="toolbar-filter-switch__control"
            inline-prompt
            :active-text="t('mesChamferingActual.filterSwitchOn')"
            :inactive-text="t('mesChamferingActual.filterSwitchOff')"
          />
        </div>
        ...
        >
          ...
          {{ t('mesChamferingActual.loadPlans') }}
        </el-button>
      </div>
    </el-card>
    ...
    <div v-loading="loadingPlans" class="plan-board">
      <template v-if="!selectedMachineId">
        <el-empty :description="t('mesChamferingActual.emptySelectMachine')" />
      </template>
      <template v-else-if="!loadingPlans && visibleRowsByDay.length === 0">
        <el-empty :description="t('mesChamferingActual.emptyNoPlans')" />
      </template>
      <div v-else class="plan-list">
        ...
        >
          <header class="plan-day-group__head">
            <div class="plan-day-group__title-row">
              ...
              <el-tag v-if="grp.isAnchorDay" size="small" effect="dark" type="primary" class="plan-day-group__badge">
                {{ t('mesChamferingActual.anchorDayBadge') }}
              </el-tag>
            </div>
            <span class="plan-day-group__count">{{ grp.rows.length }} {{ t('mesChamferingActual.planLinesSuffix') }}</span>
          </header>
          ...
          <el-card
            ...
            shadow="hover"
            class="plan-row-card"
            :class="{ 'plan-row-card--confirmed': isChamferingRowConfirmedForDisplay(row) }"
          >
          <div class="plan-row">
            <div class="plan-row__head">
              <span
                class="plan-row-card__drag-handle"
                :title="t('mesChamferingActual.dragToReorder')"
                aria-hidden="true"
              >
                <el-icon :size="16"><Rank /></el-icon>
              </span>
              <span class="plan-seq-pill">{{ t('mesChamferingActual.seq') }} {{ row.production_sequence ?? '—' }}</span>
              ...
              >
                ...
                {{ t('mesChamferingActual.btnScanCode') }}
              </el-button>
              <div
                v-if="
              ...
              >
                ...
                {{ t('mesChamferingActual.btnEditConfirmed') }}
              </el-button>
            </div>
            ...
            <div class="plan-row__meta">
              ...
              <div class="plan-meta-chips">
                <span
                  ...
                  :class="{ 'plan-chip--qty-actual': isChamferingRowConfirmedForDisplay(row) }"
                >
                ...
                <div class="plan-meta-field plan-meta-field--operator">
                  <span class="plan-meta-field__label">
                    ...
                    {{ t('mesChamferingActual.operator') }}
                  </span>
                  <el-select
                    ...
                    clearable
                    teleported
                    :disabled="isChamferingRowConfirmedForDisplay(row)"
                    :placeholder="t('mesChamferingActual.operatorPlaceholder')"
                ...
                <div class="plan-meta-field plan-meta-field--setup">
                  <span class="plan-meta-field__label">
                    ...
                    {{ t('mesChamferingActual.setupTime') }}
                    <span class="plan-meta-field__unit">（{{ t('mesChamferingActual.setupTimeUnit') }}）</span>
                  </span>
                  <el-input-number
            ...
            <div class="plan-row__timer">
              ...
              >
                ...
                <div class="timer-compact__readout-row">
                  ...
                  >
                    <span class="timer-compact__pause-label">{{ t('mesChamferingActual.pausedAccum') }}</span>
            ...
            <div class="plan-row__actions">
              ...
              >
                ...
                {{ t('mesChamferingActual.btnStart') }}
              </el-button>
              <el-button
              ...
              >
                ...
                {{ t('mesChamferingActual.btnPause') }}
              </el-button>
              <el-button
              ...
              >
                <el-icon><VideoPlay /></el-icon>
                {{ t('mesChamferingActual.btnResume') }}
              </el-button>
              ...
              >
                <el-icon><SetUp /></el-icon>
                {{ t('mesChamferingActual.btnChangeMachine') }}
              </el-button>
            </div>
            </div>
    ...
    <el-dialog
      ...
      :title="t('mesChamferingActual.endDialogTitle')"
      width="480px"
      append-to-body
      destroy-on-close
      align-center
    ...
    >
      <div v-if="endDialogRow" class="end-dialog-body">
        ...
        <el-alert
          ...
          :closable="false"
          show-icon
          :title="t('mesChamferingActual.endDialogBaselineZero')"
        />
        <el-alert
          ...
          :closable="false"
          show-icon
          :title="t('mesChamferingActual.endDialogMetaMissingHint')"
        />
        <div class="end-dialog-meta">
          <div class="end-dialog-meta-item">
            <span class="end-dialog-meta-label">{{ t('mesChamferingActual.operator') }}</span>
            <span v-if="endDialogHasOperator" class="end-dialog-meta-value">{{ endDialogOperatorLabel }}</span>
            <span v-else class="end-dialog-meta-missing">{{ t('mesChamferingActual.endDialogOperatorMissing') }}</span>
          </div>
          <div class="end-dialog-meta-item">
            <span class="end-dialog-meta-label">{{ t('mesChamferingActual.setupTime') }}</span>
            <span v-if="endDialogHasSetupTime" class="end-dialog-meta-value">
              {{ endDialogSetupTimeLabel }} {{ t('mesChamferingActual.setupTimeUnit') }}
            </span>
            <span v-else class="end-dialog-meta-missing">{{ t('mesChamferingActual.endDialogSetupTimeMissing') }}</span>
          </div>
        </div>
        <div class="end-dialog-field">
          <span class="end-dialog-label">{{ t('mesChamferingActual.endDialogQtyLabel') }}</span>
          <el-input
            v-model="endDialogQty"
            class="end-dialog-input"
            inputmode="numeric"
            :placeholder="t('mesChamferingActual.endDialogQtyPlaceholder')"
            clearable
        ...
        <div v-if="endDialogBaseline > 0" class="end-dialog-defer-settings">
          <div class="end-dialog-field">
            <span class="end-dialog-label">{{ t('mesChamferingActual.deferNextDayLabel') }}</span>
            <el-date-picker
          ...
          <p v-if="endDialogDeferRemainder > 0" class="end-dialog-defer-remainder">
            {{ t('mesChamferingActual.deferRemainderHint', { n: endDialogDeferRemainder }) }}
          </p>
          <el-checkbox
          ...
          >
            {{ t('mesChamferingActual.deferSubsequentLabel', { n: endDialogSubsequentDeferCount }) }}
          </el-checkbox>
        </div>
        <div class="end-dialog-actions">
          ...
          >
            {{ t('mesChamferingActual.btnCompleteFull', { n: endDialogBaseline }) }}
          </el-button>
          <el-button
            v-if="endDialogBaseline > 0"
          ...
          >
            {{ t('mesChamferingActual.btnDeferConfirm') }}
          </el-button>
          <el-button
            v-if="endDialogBaseline <= 0"
            class="end-dialog-btn end-dialog-btn--full"
          ...
          >
            {{ t('mesChamferingActual.btnCompleteWithInput') }}
          </el-button>
    ...
    >
          ...
          <div class="change-machine-header__main">
            ...
            <span class="change-machine-header__text">
              <span class="change-machine-header__title">{{
                t('mesChamferingActual.changeMachineDialogTitle')
              }}</span>
              <span class="change-machine-header__sub">{{
                t('mesChamferingActual.changeMachineDialogSub')
              }}</span>
            </span>
          </div>
        </div>
      ...
      <div v-if="changeMachineRow" class="change-machine-body">
        ...
        <div class="change-machine-flow">
          <div class="change-machine-card change-machine-card--current">
            <span class="change-machine-card__label">{{
              t('mesChamferingActual.changeMachineCurrent')
            }}</span>
          ...
          <div class="change-machine-card change-machine-card--target">
            <span class="change-machine-card__label">{{
              t('mesChamferingActual.changeMachineTarget')
            }}</span>
            <el-select
        ...
        <p class="change-machine-hint">{{ t('mesChamferingActual.changeMachineDialogHint') }}</p>
        <div class="change-machine-actions">
          ...
          >
            {{ t('mesChamferingActual.btnChangeMachineSave') }}
          </el-button>
          <el-button
      ...
      <template #header>
        <div class="confirmed-edit-header">
          <div class="confirmed-edit-header__main">
            ...
            <span class="confirmed-edit-header__text">
              <span class="confirmed-edit-header__title">{{
                t('mesChamferingActual.confirmedEditDialogTitle')
              }}</span>
              <span class="confirmed-edit-header__sub">{{ t('mesChamferingActual.cmConfirmed') }}</span>
            </span>
          </div>
          <el-button
          ...
          >
            ...
            {{ t('mesChamferingActual.btnClearMesActual') }}
          </el-button>
        </div>
      </template>
      <div v-if="confirmedEditRow && confirmedEditForm" class="confirmed-edit-body">
        <div class="confirmed-edit-hero">
          <span class="confirmed-edit-badge">{{ t('mesChamferingActual.cmConfirmed') }}</span>
          <span
        ...
        <el-form label-position="top" size="small" class="confirmed-edit-form">
          <section class="confirmed-edit-section confirmed-edit-section--people">
            <h4 class="confirmed-edit-section__title">
              ...
              {{ t('mesChamferingActual.operator') }} / {{ t('mesChamferingActual.actualQty') }}
            </h4>
            <el-form-item :label="t('mesChamferingActual.operator')" class="confirmed-edit-form-item">
            <el-select
          ...
          <div class="confirmed-edit-form-row">
            <el-form-item :label="t('mesChamferingActual.actualQty')" class="confirmed-edit-form-item">
              <el-input-number
            ...
            <el-form-item
              :label="`${t('mesChamferingActual.setupTime')}（${t('mesChamferingActual.setupTimeUnit')}）`"
          ...
          <section class="confirmed-edit-section confirmed-edit-section--time">
            <h4 class="confirmed-edit-section__title">
              ...
              {{ t('mesChamferingActual.productionStart') }} / {{ t('mesChamferingActual.elapsed') }}
            </h4>
            <div class="confirmed-edit-form-row">
          <el-form-item :label="t('mesChamferingActual.productionStart')" class="confirmed-edit-form-item">
            <el-date-picker
          ...
          <el-form-item :label="t('mesChamferingActual.productionEnd')" class="confirmed-edit-form-item">
            <el-date-picker
            ...
            <el-form-item
              :label="`${t('mesChamferingActual.pausedAccum')}（${t('mesChamferingActual.pausedAccumUnit')}）`"
              class="confirmed-edit-form-item"
            >
              <el-input-number
            ...
            <el-form-item :label="t('mesChamferingActual.elapsed')" class="confirmed-edit-form-item">
          ...
          <div class="confirmed-edit-hint" role="note">
            ...
            <span>{{ t('mesChamferingActual.confirmedEditPauseHint') }}</span>
          </div>
        </el-form>
      </div>
      <template #footer>
        <div class="confirmed-edit-footer">
          ...
          >
            {{ t('mesChamferingActual.btnSaveConfirmed') }}
          </el-button>
        </div>
      </template>
    </el-dialog>
