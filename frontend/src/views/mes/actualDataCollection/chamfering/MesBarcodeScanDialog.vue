...
const props = defineProps<{
  modelValue: boolean
  productLabel?: string
  /** 鐧婚尣娓堛伩銇鍙栥偝銉笺儔锛堝啀瑾彇銇т笂鏇搞亶锛?*/
  existingScannedCode?: string
...
<template>
  ...
  >
    ...
    <div v-if="(existingScannedCode ?? '').trim()" class="mes-barcode-scan-dialog__registered-row">
      <ScanRegisteredHint
        :scanned-code="(existingScannedCode ?? '').trim()"
...
.mes-barcode-scan-dialog__registered-row {
  margin: 0 0 8px;
}
