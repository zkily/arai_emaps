...
function reportFileBase(filters: CuttingProductivityReportFilters): string {
  return `切断生産性分析_${filters.startDate}_${filters.endDate}`
}
...
export function buildSummaryCsv(
  ...
  lines.push(csvLine(['ライン', '件数', '生産数', '不良率', '平均能率(本/時)', '正味稼働(分)']))
  ...
  lines.push(csvLine(['CD', '製品名', 'TOPライン', '能率(本/時)', '対象人数', '生産数']))
...
<head>
  <meta charset="UTF-8" />
  <title>切断工程 — 生産性分析${options.sectionTitle ? ` — ${options.sectionTitle}` : ''}</title>
...
<body>
  ...
  ${kpiHtml}
  ${options.body}
  <footer class="ft">Smart-EMAPs · 切断生産性分析 · ${escHtml(printedAt)}</footer>
</body>
</html>`
}
...
function buildFullPrintBody(ctx: CuttingProductivityReportContext): string {
  ...
  const operatorBadges = [
    ...
    lineSectionAvgEfficiency != null
      ? panelBadge(`平均能率 ${fmtEfficiency(lineSectionAvgEfficiency)} 本/時`, 'inspector')
      : '',
  ].join('')
...
<head>
  <meta charset="UTF-8" />
  <title>切断工程 — 生産性分析 — 日別推移（ライン別）</title>
...
<head>
  <meta charset="UTF-8" />
  <title>切断工程 — 生産性分析 — ライン別製品別</title>
...
export function exportCuttingSessionsCsv(
  data: CuttingProductivityAnalysisData,
  filters: CuttingProductivityReportFilters,
) {
...
export function exportCuttingSummaryCsv(
  data: CuttingProductivityAnalysisData,
  filters: CuttingProductivityReportFilters,
