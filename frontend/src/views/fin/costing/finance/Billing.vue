      ...
      <el-tab-pane label="請求書発行" name="billing">
        ...
        >
          ...
          <el-table-column prop="status" label="ステータス" width="100" align="center">
            <template #default="{ row }">
              <el-tag :type="getBillingStatusType(row.status)">
          ...
          <el-table-column label="操作" width="120" fixed="right">
            <template #default="{ row }">
              <el-button size="small" type="primary" link @click="handleViewBilling(row)">
                詳細
              </el-button>
            </template>
          </el-table-column>
      ...
      <el-tab-pane label="入金消込" name="receipt">
        ...
        >
          ...
          <el-table-column prop="unmatched_amount" label="未消込" width="100" align="right">
            <template #default="{ row }">
              <span v-if="row.unmatched_amount" class="text-danger">
                ¥{{ row.unmatched_amount?.toLocaleString() }}
              </span>
              <span v-else>-</span>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="100" fixed="right">
            <template #default="{ row }">
              <el-button size="small" type="primary" link @click="handleViewReceipt(row)">
                詳細
              </el-button>
            </template>
          </el-table-column>
...
const handleReceiptSelectionChange = (rows: any[]) => {
  selectedReceipts.value = rows
}
const handleCreateBilling = () => {
  ElMessage.info('締め請求書作成画面を開きます')
}
const handlePrintBilling = () => {
  ElMessage.info(`${selectedRows.value.length}件の請求書を印刷します`)
}
const handleAutoReconcile = () => {
  ElMessage.success('自動消込を実行しました')
}
const handleManualReconcile = () => {
  ElMessage.info('手動消込画面を開きます')
}
const handleViewBilling = (row: any) => {
  ElMessage.info(`請求書 ${row.invoice_no} の詳細`)
}
const handleViewReceipt = (row: any) => {
  ElMessage.info('入金詳細')
}

const getBillingStatusType = (s: string): ElTagType =>
  (({
    draft: 'info',
    sent: 'primary',
    partial: 'warning',
...
const getBillingStatusLabel = (s: string) =>
  ({ draft: '下書き', sent: '送付済', partial: '一部入金', paid: '入金済', overdue: '延滞' })[s] ||
  s
</script>
