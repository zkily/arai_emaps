...
- 存在混合文件：`046_welding_receiving_good_qty_trigger_verified.md`（说明文档，不是迁移）。
- 存在重复编号：`082_material_stock_sub_current_stock_trigger.sql` 与 `082_production_summarys_add_sw_plan.sql`。
...
- 将非 SQL 文档迁移到 `backend/database/docs/`，迁移目录只保留可执行脚本。
- 统一重复编号（例如将后出现的 `082` 重命名为 `087` 之后的空号），并在变更说明中记录映射。
