...
import type {
  ...
  InspectionUtilizationInspectorRow,
} from '@/api/inspectionManagement'

export interface InspectionUtilizationReportFilters {
  startDate: string
  endDate: string
...
export type InspectionUtilizationKpiTone = 'green' | 'blue' | 'indigo' | 'amber' | 'violet'

export interface InspectionUtilizationKpiCardPrint {
  label: string
  value: string
  hint: string
  tone: InspectionUtilizationKpiTone
}

export interface InspectionUtilizationPrintCharts {
  daily: string | null
  overtime: string | null
}

export interface InspectionUtilizationReportContext {
  filters: InspectionUtilizationReportFilters
  kpiCards: InspectionUtilizationKpiCardPrint[]
  charts: InspectionUtilizationPrintCharts
  inspectorRows: InspectionUtilizationInspectorRow[]
  dailyDetailRows: InspectionUtilizationDailyInspectorRow[]
}
...
function kpiCardHtml(card: InspectionUtilizationKpiCardPrint): string {
...
const PRINT_INSPECTOR_TABLE_MAX = 12
const PRINT_DAILY_TABLE_MAX = 16

function buildInspectorPrintTable(rows: InspectionUtilizationInspectorRow[], maxRows = 0): string {
  ...
  return `<table class="data data--inspector data--compact">
    <thead>${tableHead(['#', '検査員', '出勤日', '件', '正味(h)', '所定内(h)', '残業(h)', '稼働率', 'ｶﾚﾝﾀﾞｰ'])}</thead>
    <tbody>${body}</tbody>
  </table>${more}`
}

function buildDailyDetailPrintTable(rows: InspectionUtilizationDailyInspectorRow[], maxRows = 0): string {
  if (!rows.length) return '<p class="empty">データなし</p>'
  const display = maxRows > 0 ? rows.slice(0, maxRows) : rows
  const body = display
    .map(
  ...
  return `<table class="data data--daily data--compact">
    <thead>${tableHead(['生産日', '検査員', '件', '所定(h)', '正味', '所定内', '残業', '稼働率'])}</thead>
    <tbody>${body}</tbody>
  </table>${more}`
}
...
<head>
  <meta charset="UTF-8" />
  <title>検査工程 — 稼働率分析${options.sectionTitle ? ` — ${options.sectionTitle}` : ''}</title>
...
function buildFullPrintBody(ctx: InspectionUtilizationReportContext): string {
  const { charts, inspectorRows, dailyDetailRows } = ctx
  const inspectorBadges = [panelBadge(`${inspectorRows.length} 名`, 'inspector')].join('')
  ...
  <div class="tables-row">
    ${panelSection('検査員別サマリ', {
      theme: 'inspector',
      badges: inspectorBadges,
      tableHtml: buildInspectorPrintTable(inspectorRows, PRINT_INSPECTOR_TABLE_MAX),
    })}
    ${panelSection('検査員 × 日別明細', {
      theme: 'daily',
      badges: dailyBadges,
...
export function buildInspectionUtilizationPrintHtml(ctx: InspectionUtilizationReportContext): string {
...
export function buildInspectionUtilizationDailyPrintHtml(ctx: InspectionUtilizationReportContext): string {
  return buildPrintDocumentShell(ctx.filters, {
...
export function buildInspectionUtilizationDailyBatchPrintHtml(
  filters: InspectionUtilizationReportFilters,
...
export function printInspectionUtilizationReport(ctx: InspectionUtilizationReportContext) {
  openPrintDocument(buildInspectionUtilizationPrintHtml(ctx))
}

export function printInspectionUtilizationDailySection(ctx: InspectionUtilizationReportContext) {
  openPrintDocument(buildInspectionUtilizationDailyPrintHtml(ctx))
}

export function printInspectionUtilizationDailyBatch(
  filters: InspectionUtilizationReportFilters,
  items: UtilizationDailyBatchItem[],
) {
  openPrintDocument(buildInspectionUtilizationDailyBatchPrintHtml(filters, items))
}
